...
4. Invoke `/open` → execute plan steps → invoke `/verify` per edit → invoke `/close`. No shortcuts.
...
10. Never execute workflow steps manually unless explicitly requested by user. Always invoke skills using slash commands (e.g., `/close`, `/open`, `/verify`).
