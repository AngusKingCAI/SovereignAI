        ...
        try:
            ...
            if engine:
                rule_results = engine.evaluate_rules(
                    "PreToolUse", payload, state_machine
                )
            ...
            log_execution(
                "PreToolUse", {"event": "no_rule_match", "action": "return_none"}
            )
            return None
            ...
            if FAIL_CLOSED:
                ...
                return self._build_deny_response(
                    reason=f"Governor error in PreToolUse handler: {e} (fail-closed mode active)"
                )
            else:
