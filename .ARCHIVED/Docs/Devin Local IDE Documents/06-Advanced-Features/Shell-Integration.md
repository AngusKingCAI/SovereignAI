...
**Note**: Shell integration is separate from the `devin setup` wizard. Running `devin setup` does **not** install shell integration — you must run `devin shell setup` separately.
...
**Note**: Comment syntax requires Zsh's `INTERACTIVE_COMMENTS` option, which shell integration enables automatically.
...
**Note**: After changing configuration, run `devin shell setup` again and restart your terminal for changes to take effect.