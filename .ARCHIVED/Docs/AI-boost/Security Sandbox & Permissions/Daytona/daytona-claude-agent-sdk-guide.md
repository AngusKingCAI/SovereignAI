# Daytona Claude Agent SDK Guide

**Source URL:** https://www.daytona.io/docs/en/guides/claude/claude-agent-sdk-interactive-terminal-sandbox/
...
2. In `src/index.ts`, swap the `ANTHROPIC_API_KEY` env var for a `secrets:` mapping:
...
6. Waits for user input and sends prompts to the agent in the code interpreter context.

### Main Program Code

Once the agent is running, the program creates a readline interface to read user input and sends it to the agent. Each user request is passed to the agent by running a Python command in the code interpreter context:
...
The sandbox agent uses the [Claude Agent SDK](https://platform.claude.com/docs/en/agent-sdk/overview) to create a customized coding agent based on Claude Code. The agent is initialized with a system prompt that includes the workspace directory and an example of the [preview URL format](/docs/en/preview):
...
This guide provides a comprehensive approach to building autonomous coding agents using the Claude Agent SDK within secure Daytona sandbox environments.
