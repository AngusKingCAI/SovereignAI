...
S0.4 best practices: Single-call structured output with Pydantic validation + bounded retry (max 3) gives best reliability/speed tradeoff for local 7B models. Token-budget history prevents context window overflow. ReAct loop as CapabilityGraph-registered module preserves P1.
...
- On StructuredOutputExhaustedError: (1) close tool connections via ToolSessionRegistry, (2) drain remaining queue (discard pending tool calls), (3) emit AgentErrorObservation and return. ReActLoop emits trace.react.structured_output_exhausted via injected TraceEmitter before cleanup.
...
- test_structured_output.py: validation + retry, max 3 attempts, exhausted retry returns typed error.
