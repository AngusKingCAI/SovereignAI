...
Open questions resolved: Q-24.2 (CapabilityGraph skill registration)
...
- run() resolves skill_id via CapabilityGraph.find_providers(CapabilityCategory.SKILL, skill_name) where skill_name is derived from skill_id. If multiple providers match, use the first. If none match, raise SkillNotFoundError.
...
- to_component_manifest() method returns ComponentManifest with: component_id=self.id, display_name=self.name, version=self.version, category=CapabilityCategory.SKILL, capabilities=self.capabilities.
...
- Register discovered capabilities in existing CapabilityGraph (sovereignai/shared/capability_graph.py) via to_component_manifest(). Use CapabilityGraph.register(manifest=component_manifest, instance=None) — instance is deferred to ConcreteSkillRunner.run().
- Maintain internal _skill_index: dict mapping skill_id → (CapabilityCategory.SKILL, skill_name) for lookup by ConcreteSkillRunner.
...
- Register CapabilityGraph as singleton (already exists in repo; ensure it is registered if not already). Use try/except KeyError to avoid duplicate registration: try container.retrieve(CapabilityGraph); except KeyError: container.register_singleton(CapabilityGraph, CapabilityGraph()).
...
- test_skill_manifest.py: manifest parsing, no-import guarantee, sys.modules unchanged, dag_parse_simple, dag_parse_missing_optional, test_to_component_manifest_mapping.
