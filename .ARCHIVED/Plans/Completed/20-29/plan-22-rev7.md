...
### S0.5 — Create TraceEmitter in sovereignai/observability/trace_emitter.py
- Thin wrapper over EventBus that publishes trace.* events.
- Method: emit(event_name: str, payload: dict) -> None. Prefixes event_name with "trace." and publishes via injected EventBus.
- Wire as singleton in build_container(). Inject EventBus into constructor.
- Existing TraceEmitter from prompt-20.9.9 is extended; verify it supports new event types: trace.react.structured_output_exhausted, trace.department.context_build_failed, trace.handler_crash, queue.dropped.
...
- On construction, register the injected skill_runner in session_registry under a unique per-instance key: session_registry.register(f"default-{id(self)}", skill_runner). This ensures cleanup (S5) closes only this loop's runner without affecting other instances.
- State: _pending_tool_calls: asyncio.Queue = asyncio.Queue() (for drain in S5).
...
- close_all() iterates registered runners and calls runner.close() (optional, default no-op per ISkillRunner protocol).
...
- Safety bounds: max_iterations=50, max_consecutive_errors=5, max_execution_time=600s (enforced via asyncio.timeout wrapping the run() call).
- On StructuredOutputExhaustedError: (1) close tool connections via ToolSessionRegistry.close_all(), (2) drain _pending_tool_calls queue (discard all items), (3) emit AgentErrorObservation and return. ReActLoop emits trace.react.structured_output_exhausted via injected TraceEmitter before cleanup.
...
- test_react_concurrency.py: two ReActLoop instances running concurrently with shared LLM backend (mock) do not interfere. Retry counters are per-instance. test_error_cleans_up_resources (verifies close → drain → emit order). test_duplicate_registration_raises (verifies ValueError on different runner, same key).
- test_agent_api.py: endpoint auth, task submission, SSE stream auth rejection.
- test_trace_emitter.py: verifies TraceEmitter emits prefixed events via EventBus.

### S9 — Wire in main.py build_container()
- Register ReActLoop as FACTORY (not singleton) under capability name 'agent.react'.
- Register ReActLoopFactory: type alias Callable[[], ReActLoop]. Use register_factory(ReActLoopFactory, lambda: ReActLoop(...)) where ... are injected dependencies from container.
