...
Open questions resolved: Q-23.1 (critical event timeout), Q-23.2 (queue.dropped feedback loop), Q-23.3 (lifespan ordering)
...
- In existing lifespan() in web/main.py (lines 44-53): wrap FULL lifecycle in try/finally. Exact order:
  ...
  ```
- EventBus.start() MUST precede container.start() so that any component startup events are routable. Shutdown is reverse order.
...
- EventBus.start() captures self._loop via asyncio.get_running_loop() and stores self._loop_thread = threading.current_thread(). EventBus.stop() cancels drain tasks and awaits them.
- EventBus.stop() is idempotent. Guard all internal collections: if not self._started: return. Initialize empty defaults in __init__.
- If publish() or publish_async() called before start(): raise RuntimeError('EventBus not started. Call start() before publishing.'). No silent no-op or buffering.
...
- Dedicated asyncio drain task per handler. Drain task wraps handler call in try/except; on unhandled exception, log error, emit trace.handler_crash event via _publish_internal() (bypasses post-stop guard), and continue draining. Do not stop the drain task.
...
- CRITICAL EVENT EXEMPTION: Events with trace_level=CRITICAL or event_type ending in .error or .completed are exempt from drop-newest. If the queue is full and the incoming event is critical, block with 5-second timeout via asyncio.wait_for(queue.put(event), timeout=5.0). On asyncio.TimeoutError: write event to critical_overflow.sqlite and emit trace.critical_overflow event. The publisher never blocks longer than 5 seconds.
- queue.dropped events are themselves exempt from normal drop-newest — they use the critical-event path (5s timeout + SQLite overflow on failure). This breaks the feedback loop where dropped events generate more drops.
- Drain tasks must never call publish() synchronously from within their own callback. Any trace emission from drain uses create_task() or _publish_internal() bypass.
...
- test_async_delivery.py: FIFO order, circuit breaker trigger, sync publish same-thread (create_task), sync publish cross-thread (run_coroutine_threadsafe), queue_full_drops_newest, test_critical_event_exemption (verifies .error/.completed events use 5s timeout, not indefinite block), test_librarian_never_drops_events, test_drain_task_crash_recovery, test_publish_before_start_raises, test_publish_after_stop_raises, test_queue_dropped_breaks_feedback_loop (verifies queue.dropped uses critical path).
