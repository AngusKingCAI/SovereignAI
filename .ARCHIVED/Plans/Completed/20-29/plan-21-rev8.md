...
- Register CapabilityGraph as singleton (already exists in repo; ensure it is registered if not already). Check before retrieve to avoid broad exception catch: if CapabilityGraph not in container._instances: container.register_singleton(CapabilityGraph, CapabilityGraph(trace=container.retrieve(TraceEmitter))). Do not use try/except KeyError.
