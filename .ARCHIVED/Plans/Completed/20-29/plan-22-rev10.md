...
- Plan 24's GraphMemoryBackend (or TaskGraphCache) satisfies this Protocol.
...
- test_react_loop_factory_protocol_registration: verifies Protocol class works as DIContainer key; if rejected, concrete fallback works.
- test_graph_memory_protocol: verifies duck-typed GraphMemory object satisfies Protocol.
...
- Define ReActLoopFactory as Protocol class (not type alias):
  ...
  ```
- Primary registration: container.register_factory(ReActLoopFactory, lambda: ReActLoop(...)) where ... are injected dependencies from container.
- Fallback: if DIContainer rejects Protocol class as key, register against concrete ReActLoop type: container.register_factory(ReActLoop, lambda: ReActLoop(...)). Both registrations use same factory lambda.
