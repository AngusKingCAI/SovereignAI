...
class LifecycleManager:
    ...
    def get_status(self, component_id: ComponentId) -> ComponentStatus:
        ...
        Per Finding 2 (Rev2): auto-recovers CIRCUIT_BROKEN components
        if the error window has expired. If no errors have occurred in
        the last CIRCUIT_BREAKER_WINDOW_SECONDS, the component flips
        back to ACTIVE.
...
class TaskStateMachine:
    ...
    def submit(self, task: Task, dag: Optional["DAGSpec"] = None) -> None:
        ...
        Args:
            ...
            dag: Optional DAG specification for composite tasks. If
                provided, validated via validate_dag() before submission.
    ...
    def transition(self, task_id: UUID, new_state: TaskState) -> None:
        ...
        Raises:
            InvalidStateTransitionError: If the transition is not in
                the valid transitions table, or if the task_id is
                unknown.
        """
        with self._lock:
            old_state = self._states.get(task_id)
            if old_state is None:
                raise InvalidStateTransitionError(task_id, TaskState.FAILED, new_state)
...
- sovereignai/shared/lifecycle_manager.py (new — circuit breaker per AR16, 50 errors/10s; auto-recovery per Finding 2; emits ERROR trace per Finding 4; set_status method per Finding 6)
- sovereignai/shared/routing_engine.py (new — capability-based routing, skips non-ACTIVE)
- sovereignai/shared/task_state_machine.py (new — ITaskStateQuery protocol, in-memory only per A7; raises InvalidStateTransitionError per Finding 3; submit() validates DAG per Finding 1; get_state returns None for unknown per Finding 5; now_utc_safe removed per Finding 7)
...
*Plan 3 — Execution Layer. Rev2. Architect draft. Part of Plans 1-4 batch — Round Table reviews alongside plan-1-Rev2, plan-2-Rev2, plan-4-Rev2, and the shared context brief. Adjudication log above records Rev1 → Rev2 changes per GR4.*
