...
class CapabilityAPI:
    ...
    def submit_task(self, token: str, category: CapabilityCategory,
        ...
        except (DAGValidationError, InvalidStateTransitionError, UnknownTaskError) as exc:
...
# Per Rev4 Finding 3: expanded to include ALL sovereignai.{layer} packages
# (orchestrator, managers, workers, librarian, adapters, skills). UIs may
# import ONLY from sovereignai.shared.capability_api (the public API).
UI_PACKAGE_DENYLIST = {
    "sovereignai.shared",
...
*Plan 4 — Interface Layer. Rev4. Architect draft. Part of Plans 1-4 batch — Round Table reviews alongside plan-1-Rev3, plan-2-Rev3, plan-3-Rev4, and the shared context brief. Adjudication logs above record Rev1 → Rev2 → Rev3 → Rev4 changes per GR4. Q26 (Composition Root bootstrap) confirmed at this plan's /close.*
