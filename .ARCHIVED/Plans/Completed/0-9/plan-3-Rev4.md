...
class NoActiveProviderError(Exception):
    ...
    Rev3 had duplicate definitions in routing_engine.py and types.py —
    callers catching one wouldn't catch the other.
    """
...
class RoutingEngine:
    ...
    def route(self, category: CapabilityCategory, name: str) -> ComponentId:
        ...
        for component_id, _declaration in providers:
            # Per Rev4 Finding 1: attempt recovery before checking status
            self._lifecycle.try_recover(component_id)
...
class LifecycleManager:
    ...
    def try_recover(self, component_id: ComponentId) -> bool:
        """Attempt to recover a CIRCUIT_BROKEN component if the error window has expired.

        Per Finding 7 (Rev3): recovery is now an explicit operation,
        not a side effect of get_status(). Callers (e.g. the future
        RoutingEngine, or a periodic heartbeat) invoke this to check
        whether a circuit-broken component is ready to retry.

        Per Finding 7 (Rev3): emits an INFO trace on successful recovery
        so operators can see the recovery event in the Log drawer (P9
        observability — Rev2's silent recovery was a P9 violation).

        Args:
            component_id: The component to attempt recovery on.

        Returns:
            True if the component was recovered (status flipped from
            CIRCUIT_BROKEN to ACTIVE), False otherwise.
        """
        now = monotonic()
        recovered = False
        with self._lock:
            status = self._statuses.get(component_id)
            if status is not ComponentStatus.CIRCUIT_BROKEN:
                return False
        ...
        if recovered:
            self._trace.emit(
                ...
                message=(
                    f"Component {component_id} automatically recovered to ACTIVE "
                    f"(error window expired)"
                ),
            )
        return recovered
...
class UnknownTaskError(Exception):
    ...
    Per Finding 11 (Rev3): distinguishes "task never existed" from
    "task existed but transition is invalid." Rev2 conflated these by
    raising InvalidStateTransitionError with FAILED as old_state for
    unknown tasks — misleading.
    """
    ...
    def __init__(self, task_id) -> None:
        ...
        Args:
            task_id: UUID of the task that was not found.
        """
...
class InvalidStateTransitionError(Exception):
    ...
    Per Finding 3 (Rev2): invalid transitions now raise instead of being
    silently logged. The error includes the task_id, old_state, and
    attempted new_state for diagnosis. The task's state is NOT modified.
    """

    def __init__(self, task_id, old_state: TaskState, new_state: TaskState) -> None:
        """Create an error describing an invalid state transition attempt.

        Args:
            task_id: UUID of the task that could not transition.
            old_state: The task's current valid state.
            new_state: The state that was attempted (and rejected).
        """
        self.task_id = task_id
...
*Plan 3 — Execution Layer. Rev4. Architect draft. Part of Plans 1-4 batch — Round Table reviews alongside plan-1-Rev3, plan-2-Rev3, plan-4-Rev4, and the shared context brief. Adjudication logs above record Rev1 → Rev2 → Rev3 → Rev4 changes per GR4.*
