...
class CapabilityAPI:
    ...
    def submit_task(self, token: str, category: CapabilityCategory,
        ...
        # errors to CapabilityAPIError. For atomic tasks (dag=None),
        # DAGValidationError cannot occur. The catch is defensive for
        # future composite task submission.
        try:
            self._state_machine.submit(task)
        except Exception as exc:
            # Catch DAGValidationError, InvalidStateTransitionError,
            # UnknownTaskError — re-raise as CapabilityAPIError so
            # callers get a single typed exception.
...
# The Capability API itself is allowed to import from these (it lives in
# shared/ and constructs Task objects from types.py).
UI_PACKAGE_DENYLIST = {
    "sovereignai.shared",
...
def test_ui_directories_do_not_import_core_internals(ui_dir: str) -> None:
    ...
    for py_file in py_files:
        ...
        forbidden_concrete = imports & CORE_INTERNALS_CONCRETE_DENYLIST
        forbidden_package = imports & UI_PACKAGE_DENYLIST
        ...
        assert not forbidden_package, (
            ...
            f"Per AR7, UIs must not import sovereignai.shared or "
            f"sovereignai.shared.types directly."
        )
```
...
**Files changed**:
- sovereignai/shared/types.py (extended: SessionToken, AuthError, CapabilityQuery, CapabilityResponse, RelayNotSupportedError per Finding 5, NoActiveProviderError per Rev3 Finding 4, CapabilityAPIError per Rev3 Finding 9)
- sovereignai/shared/auth.py (new — PBKDF2 hashing, session tokens, 8h TTL)
- sovereignai/shared/capability_api.py (new — public contract for UIs; submit_task calls state_machine.submit() per Finding 1; accepts category param per Finding 2; validates capability exists per Finding 3; imports TaskStateMachine concrete for submit per Finding 1; wraps submit in try/except per Rev3 Finding 9; docstring fixed per Rev3 Finding 13)
...
- tests/test_ar7_no_core_imports_in_ui.py (new — AR7 enforcement per Finding 4 + Rev3 Findings 5/8: catches from...import sub-module forms; denylist excludes protocol modules; UI_PACKAGE_DENYLIST for sovereignai.shared + types)
...
- DEBT.md (add task TTL/eviction deferral per Rev3 Finding 14)
- DECISIONS.md (add D5: accepted AR7 gap for TaskStateMachine import per Rev3 Finding 2/3)
...
*Plan 4 — Interface Layer. Rev3. Architect draft. Part of Plans 1-4 batch — Round Table reviews alongside plan-1-Rev3, plan-2-Rev3, plan-3-Rev3, and the shared context brief. Adjudication logs above record Rev1 → Rev2 → Rev3 changes per GR4. Q26 (Composition Root bootstrap) confirmed at this plan's /close.*
