...
Per GR4. 6 panelist responses (5 CLEAN PASS, 1 CONDITIONAL PASS). MiniMax-M3 raised 4 conditions — 3 accepted as specification gaps, 1 as DEBT.

### Finding 1 — `/api/auth/status` endpoint referenced by Plan 8 but not defined (MiniMax Cross-Issue 1 — HIGH)
**Action**: ACCEPTED. Add `/api/auth/status` endpoint to S4:
```python
@app.get("/api/auth/status")
async def auth_status(request: Request):
    """Check if session is valid. Used by SSE 401 handler."""
...
```
This endpoint is NOT protected by `Depends(get_current_user)` — it does its own check and returns 401 or 200. It's in the `/api/auth/*` allowlist so first-run middleware doesn't block it.

### Finding 2 — Dispatcher token parameter source/validation unspecified (MiniMax Cross-Issue 2 — MEDIUM)
**Action**: ACCEPTED. Plan 7 S7 `/api/dispatch` endpoint must extract the session cookie and pass it:
```python
...
```
The dispatcher passes `token` to `CapabilityAPI.submit_task(token, ...)` which validates it internally via `auth.validate()`. If the token is invalid, `submit_task` raises `AuthError` → 401. The dispatcher does NOT validate the token itself — that's the API's job.

### Finding 3 — `chat` skill manifest location and registration unspecified (MiniMax Cross-Issue 3 — MEDIUM)
**Action**: ACCEPTED. The Ollama adapter already has a manifest (Plan 7 S4). The `chat` skill is the Ollama adapter itself — it's not a separate skill file. The dispatcher routes to it when no other skill matches. The manifest already declares `text_generation` capability. The dispatcher's fallback logic: if `_find_matching_skill()` returns None, use the Ollama adapter's component ID directly. No separate manifest file needed.

### Finding 4 — No `tests/test_plan4_contract.py` (MiniMax Gap 1 — MEDIUM)
**Action**: ACCEPTED as DEBT. Add to DEBT.md: "Contract test for Plan 4 auth interface — verify login(), validate(), register_user(), _password_hashes exist. Target: Plan 10+."

### Verdict: All HIGH/MEDIUM issues fixed. Plan 9 Rev5 is ready for execution.
...
# Per Finding 1 (Rev5): /api/auth/status endpoint for SSE 401 handler
@app.get("/api/auth/status")
async def auth_status(request: Request):
