...
S3.7a: **`ready` field state machine**: `process_startup → ready=false` → `all_startup_stages_complete → ready=true` → `SIGTERM_or_SIGINT → ready=false` → `process_exits` (subsequent connection attempts fail). On critical startup failure: `ready` stays `false`, process exits non-zero. During graceful drain, `ready=false` is the first signal TUI/Web sees. **Security note**: endpoint exposes `server_pid` for operational monitoring. In production behind reverse proxy, restrict to internal networks. `server_pid` is not secret but could aid targeted local attacks.
...
S4.3: Shutdown sequence (sequential, after drain): Orchestrator (20s) → ModelRegistry (20s) → OptionsBackend (20s) → [emit `lifecycle.stopped` via EventBus, drain 2s] → EventBus (20s) → LifecycleHookRegistry (30s). Process exits. **Force-kill: 190s** (60 drain + 4×20s stages + 30s hooks + 20s headroom; 2s stopped-drain folded into headroom). **Rationale**: Orchestrator→ModelRegistry→Options drain in order; emit `lifecycle.stopped` before EventBus shutdown ensures SSE delivery; EventBus last; Hooks last (stderr fallback for logging after EventBus down).
...
S5.1: Create `app/sovereignai/main.py` — compose all core services via DIContainer. **7-step instantiation order**:
  1. `CoreServicesComposer` → EventBus, OptionsBackend
  2. `OrchestratorComposer` → Orchestrator, InterDepartmentBus
  3. `ModelRegistryComposer` → ModelRegistry (receives `OrchestratorFactory` callable)
  4. `AdapterComposer` → adapter registry
  5. `LifecycleComposer` → LifecycleManager, HealthAggregator, HookRegistry
  6. `MemoryComposer` → MemoryGateway (nullable until loaded), PersistentGraphMemory; registers hooks with HookRegistry: `MemoryGateway.load()` (non-critical startup), `MemoryGateway.flush()` (critical shutdown)
  7. `LifecycleManager.start()` — triggers startup hooks (Plan 34 `MemoryGateway.load()`)
...
S5.3: Sub-composers: `CoreServicesComposer` (EventBus, OptionsBackend), `OrchestratorComposer` (Orchestrator, InterDepartmentBus), `ModelRegistryComposer` (ModelRegistry), `AdapterComposer` (adapter registry), `LifecycleComposer` (LifecycleManager, HealthAggregator, HookRegistry), `MemoryComposer` (MemoryGateway, PersistentGraphMemory). MemoryGateway sole owner of PGM per Plan 34 S5.3.
