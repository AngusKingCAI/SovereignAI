...
- S1.4: load_model(model_id: str) -> None: resolve path via model_path_resolver. Wrap file header read in try/except → raise AdapterUnavailableError("Invalid or unreadable GGUF file") on OSError. Verify first 4 bytes == b'GGUF' and version field >= 2 (accepts v2 and v3; v1 deprecated). Compute VRAM budget: `gpus = hardware_probe.sample().gpus; if not gpus: n_gpu_layers = 0; trace WARN "No GPU — CPU mode"; else: vram_budget_mb = sum(g.vram_total_mb for g in gpus); n_gpu_layers = min(model.num_layers, vram_budget_mb * model.num_layers // max(1, model.vram_required_mb))`. Use high-level `llama_cpp.Llama(model_path=str(path), n_gpu_layers=n_gpu_layers)`. Emit trace on load start/complete/failure.
...
- S1.7: Add txt/requirements.txt: `llama-cpp-python>=0.2.50`. Add DEBT.md entry: "llama-cpp-python 0.2.50 — test with real binary before v1.0". Add DEBT.md entry: "generate() timeout — defer to post-Plan-19 (cross-platform timeout infra non-trivial)". Add DEBT.md entry: "health_check caching in RoutingEngine — defer to post-Plan-19 (TTL cache adds statefulness)".
...
- S1.9: Run /verify on each. Add tests/test_llama_cpp_adapter.py with llama_cpp mocked. Cover: load_model with no GPU (n_gpu_layers=0); load_model with multi-GPU (VRAM aggregated); load_model with corrupt GGUF (AdapterUnavailableError); load_model with v2 GGUF (accepted); generate failure raises AdapterUnavailableError; health_check import failure; health_check GPU-offload failure when requested; health_check healthy when CPU-only (requested_n_gpu_layers=0); health_check healthy when offload probe absent but CPU mode.
...
- S2.4: Run /verify on each. Extend tests/test_routing_engine.py: (a) failover test (Ollama raises AdapterUnavailableError, llama.cpp succeeds, verify failover trace); (b) single-adapter returns its result; (c) single-adapter raises → error propagates (no silent fallback); (d) all unhealthy → NoHealthyAdapterError; (e) health_check filter skips unhealthy adapter; (f) CPU-only llama.cpp (requested_n_gpu_layers=0) is healthy and routes successfully.
...
- S4.1: Create scripts/ar_checks/check_p4_compliance.py — mechanical scan: (a) count adapters in adapters/external/ with manifest declaring capability "model.inference"; if <2, check DEBT.md for explicit P4 exception entry (must contain literal string "P4-EXCEPTION" and a target plan); if no such entry, FAIL; (b) verify RoutingEngine tests cover failover + single-adapter + all-unhealthy + CPU-only-healthy; (c) AST scan: find Raise nodes in main.py whose exception type is AdapterNotFoundError/ServiceNotFoundError/etc. and which are NOT inside `if not _test_mode` block — forbidden. Exit≠0 on violation.
- S4.2: Edit .devin/workflows/close.md step 8 — add check_p4_compliance.py to AR checks list.
- S4.3: Run /verify. Add tests/test_p4_compliance.py. Cover: passes with 2 adapters; fails with 1 adapter and no P4-EXCEPTION; passes with 1 adapter and valid P4-EXCEPTION entry; AST scan flags unprotected Raise.
