...
# Rev9: new_correlation_id, TraceLevel, etc. are imported at the top of main.py
# (see S7 code block imports — both S6 and S7 are in the same main.py file).
...
from sovereignai.shared.types import TraceLevel, new_correlation_id, _is_valid_uuid, now_utc
...
def on_clean_shutdown(container, trace) -> None:
    ...
    Per Rev8: now_utc imported at top of S7 code block.
    """
    marker_path = os.path.expanduser("~/.sovereignai/.shutdown_marker")
