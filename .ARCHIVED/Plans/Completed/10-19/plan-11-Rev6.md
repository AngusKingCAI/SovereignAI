...
**`get_last_task_states() -> dict[str, str]`** (Per Rev6 — SQL specified to prevent misfire):
```sql
SELECT task_id, task_state FROM traces
WHERE id IN (
    SELECT MAX(id) FROM traces
    WHERE task_id IS NOT NULL AND task_state IS NOT NULL
    GROUP BY task_id
)
```
This returns the LAST recorded state for each task (by id, which is monotonically increasing). Without this specification, a naive `SELECT task_id, task_state FROM traces` would return ALL transitions — including early `received` states for completed tasks — causing recovery to mark completed tasks as failed.
