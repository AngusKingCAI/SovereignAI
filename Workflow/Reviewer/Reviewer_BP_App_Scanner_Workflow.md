...
- 1. Define scan scope: App/ directory (every single file - no exceptions)
...
- 4. **CRITICAL REQUIREMENT**: Every single file must be checked against best practices - no file may be skipped
...
- 7. **PRINT** "Scan scope defined - App/ directory comprehensive compliance verification - every file will be examined"
...
- 8. **CROSS-CHECK VALIDATION**: Compare discovered files against validation baseline to ensure no App/ directories were missed
- 9. **STATUS TRACKING**: Update workflow status to "phase_3_complete"
- 10. **PRINT** "File discovery complete - [N] files categorized by module and sorted alphabetically - pre-flight validation passed - every file will be examined against best practices in chronological order"
...
- 1. Collect all scanning results from SCAN-REPORT file (Logs/Reviewer/BP/App/SCAN-REPORT-[YYYY-MM-DD_HH-MM-SS].md)
...
- 7. **PRINT** "Findings consolidated from SCAN-REPORT - [N] issues categorized by severity across [N] files - every file examined"

### Phase 6. Compliance Report Generation
- 1. Generate comprehensive compliance report with detailed findings for every single file:
...
- 2. **CRITICAL REQUIREMENT**: Ensure report includes analysis for every single file - no file may be omitted from the report
- 3. Save report to Logs/Reviewer/BP/App/best-practice-scan-[YYYY-MM-DD_HH-MM-SS].md
- 4. **VALIDATION**: Validate that report generation completed successfully and every file is included
- 5. **STATUS TRACKING**: Update workflow status to "phase_6_complete"
- 6. **PRINT** "Compliance report generated - saved to Logs/Reviewer/BP/App/ - includes detailed analysis for every single file"
...
- 3. Save planner-ready document to Plans/Reviewer/reviewer-implementation-plan-[timestamp].md
...
- 2. **PRINT** "Compliance report available in Logs/Reviewer/BP/App/ for review and action"
