...
S1.5: Stage failure fallback: if EventBus unavailable, write to stderr + `platformdirs.user_data_dir("sovereignai") / "logs" / "lifecycle_fallback.log"` (rotating 10MB × 5). Drain on recovery.
S1.6: TUI NOT in startup — separate process per AR12. TUI polls `/api/lifecycle/ready` before connecting.
S1.7: **Route Registration Readiness Check**: polls `GET /api/lifecycle/ready` (Plan 31 mounts via Web bridge; Plan 33 supplies backend service). Does not start Web process. **Behavior**: connection success + `ready: true` = success. Connection success + `ready: false` = counts toward 30s timeout (Web is alive but not ready). Connection failure = counts toward timeout. Log progress every 10s. On timeout: continue startup degraded. On success: proceed. **Non-blocking** if Web unavailable.
...
S2.3: Hook failure behavior:
  - **Critical SHUTDOWN hook failure**: log + emit `lifecycle.hook.failed` + mark stage FAILED, continue remaining teardown stages within timeouts, write sentinel before force-kill.
  - **Non-critical hook failure** (startup or shutdown): log + emit + continue next hook.
  - **Critical STARTUP hook failure**: emit `lifecycle.hook.failed`; abort startup with `LifecycleError` raised out of `LifecycleManager.run_startup()`. Caller (S5.1 step 6) catches: degraded-start permitted only if all critical startup hooks passed. Non-critical startup hook failure: log + emit + continue.
  - All hook failures emit to stderr fallback and EventBus if available.
S2.4: **Canonical hook timeout rules** (supersedes all other refs): per-startup hook 15s each (sequential); per-shutdown: non-critical concurrent 15s shared + critical sequential 15s = 30s total. Cross-ref: S4.3 force-kill budget accounts for 30s.
S2.5: **v1 hook cap**: max 5 total startup hooks (critical + non-critical combined). Max 5 total shutdown hooks (4 non-critical + 1 critical). AR check `check_max_shutdown_hooks.py` enforces both caps. Beyond v1 cap, plans must consolidate into fewer hooks or move work to background tasks.
S2.6: Plan 34 registers: startup hook = `MemoryGateway.load()` (critical=False — system starts without memory, routes return 503 until loaded), shutdown hook = `MemoryGateway.flush()` (critical=True). **Load timeout**: if `load()` exceeds 15s hook timeout, log WARN, emit `lifecycle.memory_load_timeout`, continue with memory disabled (routes return 503). `MemoryGateway.is_ready() -> bool`: True after `load()` completes, False after `flush()` or if load not yet completed.
...
S3.7a: **`ready` field state machine**: `process_startup → ready=false` → `all_startup_stages_complete → ready=true` → `SIGTERM_or_SIGINT → ready=false` → `process_exits → 404`. On critical startup failure: `ready` stays `false`, process exits non-zero. During graceful drain, `ready=false` is the first signal TUI/Web sees. **Security note**: endpoint exposes `server_pid` for operational monitoring. In production behind reverse proxy, restrict to internal networks. `server_pid` is not secret but could aid targeted local attacks.
...
S4.1: SIGTERM/SIGINT handler: set state SHUTTING_DOWN. **Immediately**: (a) set `ready = false` (S3.7a state machine), (b) emit `lifecycle.shutting_down` via EventBus with `{timestamp, server_pid, instance_uuid, drain_timeout_seconds: 60}`. This is the primary shutdown signal for SSE consumers (Plan 32 S4.4). Hook failures do not prevent this emission.
S4.2: Drain in-flight: 60s timeout, separate pre-stage. **Drain-time route policy**:
  - `/api/lifecycle/ready`: returns `ready: false` (already set in S4.1)
  - `/api/health`: continues returning cached results (polling stopped, but last cache still valid)
  - `/api/lifecycle/stream` SSE: continues streaming `lifecycle.shutting_down` event; new SSE connections rejected with 503
  - `POST /api/auth/logout`: exempt from 503 (Plan 32 needs this for graceful disconnect)
  - `GET /api/trace/logs` REST: exempt (TUI fallback)
  - All other application routes: return HTTP 503 with `Retry-After: 60`
S4.3: Shutdown sequence (sequential, after drain): Orchestrator (20s) → ModelRegistry (20s) → OptionsBackend (20s) → [emit `lifecycle.stopped` via EventBus, drain 2s] → EventBus (20s) → LifecycleHookRegistry (30s). Process exits. **Force-kill: 190s** (60 drain + 4×20s + 30s hooks + 20s headroom). **Rationale**: Orchestrator→ModelRegistry→Options drain in order; emit `lifecycle.stopped` before EventBus shutdown ensures SSE delivery; EventBus last; Hooks last (stderr fallback).
S4.4: TUI NOT in shutdown sequence — monitors SSE `/api/lifecycle/stream` and file sentinel; exits client-side.
S4.5: Before force-kill: write sentinel at `platformdirs.user_cache_dir("sovereignai") / f"shutdown.{os.getpid()}.sentinel"` — atomic write, fsync. Emit `lifecycle.shutdown.forced` if EventBus available; stderr fallback otherwise. On next startup, cleanup deletes `shutdown.*.sentinel` files for PIDs not running (`os.kill(pid, 0)` — ProcessLookupError = not running (delete sentinel); PermissionError = running under different user (skip, log WARN)). **Corrupted sentinel** (unparseable PID): log ERROR, delete file (assume stale). **v1 limitation**: single instance per user; sentinel valid only for same process lifetime.
...
S5.1: Create `app/sovereignai/main.py` — compose all core services via DIContainer. **6-step instantiation order**:
  1. `CoreServicesComposer` → EventBus, OptionsBackend
  2. `OrchestratorComposer` → Orchestrator, InterDepartmentBus
  3. `AdapterComposer` → adapter registry
  4. `LifecycleComposer` → LifecycleManager, HealthAggregator, HookRegistry
  5. `MemoryComposer` → MemoryGateway (nullable until loaded), PersistentGraphMemory; registers hooks with HookRegistry: `MemoryGateway.load()` (non-critical startup), `MemoryGateway.flush()` (critical shutdown)
  6. `LifecycleManager.start()` — triggers startup hooks (Plan 34 `MemoryGateway.load()`)
  Orchestrator receives `Optional[MemoryGateway]` via DI. **`MemoryGatewayProvider`**: deferred dependency interface — Orchestrator does not resolve it at construction. When a memory API call arrives before binding completes, return `MemoryNotReadyResponse` (503) per Plan 31 S3.6. Memory API returns service-unavailable until `MemoryGateway.load()` completes. Test concurrent first memory requests.
S5.2: No hardcoded names per AR4; all resolved from capability graph or DI registry
S5.3: Sub-composers: `CoreServicesComposer` (EventBus, OptionsBackend), `OrchestratorComposer` (Orchestrator, InterDepartmentBus), `AdapterComposer` (adapter registry), `LifecycleComposer` (LifecycleManager, HealthAggregator, HookRegistry), `MemoryComposer` (MemoryGateway, PersistentGraphMemory). MemoryGateway sole owner of PGM per Plan 34 S5.3.
S5.4: ≤15 constructor args per sub-composer/manager/hook class (P11). AR check `test_max_constructor_args.py`.
S5.5: Test: `pytest app/sovereignai/tests/test_main_composition.py -v` — `test_6_step_instantiation_order`, `test_memory_gateway_nullable_at_construction`, `test_memory_api_503_before_load`, `test_concurrent_first_memory_requests`

## S6 — Circuit Breaker Integration

S6.1: Worker circuit breaker per AR7: >50 errors in 10s per worker (`worker_id`) = unload. Error = unhandled exception or adapter error response.
S6.2: Orchestrator circuit breaker: 10 consecutive message failures within sliding 60s window = DEGRADED. Any success resets the run. If >60s since last failure, run resets. **Success criterion**: Orchestrator-handled message returning `OrchestratorResponse` with `error is None` counts as success. Exception, validation error, clarification request, partial-success = all count as failure. Recovery: 60s cooldown, half-open probe, 3 consecutive HEALTHY polls → close.
...
S7.7: Add `check_force_kill_budget.py` — parse constants `FORCE_KILL_TIMEOUT_S`, `DRAIN_TIMEOUT_S`, `STAGE_TIMEOUT_S`, `HOOK_BUDGET_S`, `HEADROOM_S` from lifecycle constants module; assert `FORCE_KILL_TIMEOUT_S == DRAIN_TIMEOUT_S + 4*STAGE_TIMEOUT_S + HOOK_BUDGET_S + HEADROOM_S`. Edits changing one constant without updating others will fail.
