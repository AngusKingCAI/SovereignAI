...
- **OR49**: WebSocket connections authenticate via session token passed in the query parameter (`?token=...`). Cookie-based auth is incompatible with the WebSocket handshake in some browser configurations. Source: Plan 6.

Commit: `docs: add OR48-OR49 for prompt-6`
...
### S2 — Create web/main.py (FastAPI application)

Create `web/main.py` with the following structure:

1. **FastAPI app instance** with CORS configured for localhost only.
...
6. **HTTP endpoints**:
   ...
   - `GET /api/capabilities` → retrieve `CapabilityAPI` from container, call `query_capabilities()`, return JSON list of `CapabilityResponse`.
   - `POST /api/tasks` → parse JSON body `{ "message": str }`, retrieve `CapabilityAPI`, call `submit_task()` with the message wrapped in a `CapabilityQuery`, return JSON `{ "task_id": str }`.
   - `GET /api/tasks/{task_id}` → retrieve `CapabilityAPI`, call `get_task_state()`, return JSON `Task` object.
7. **WebSocket endpoint** `GET /ws`:
   - Accept WebSocket connection.
   - Subscribe to `TraceEmitter` events by registering a callback that forwards each event to the WebSocket client as JSON.
   - On disconnect: unsubscribe from TraceEmitter.
   - Token validation: read `?token=` from query params, validate via `AuthMiddleware.validate_token()`. If invalid, close connection with 1008 (policy violation).
...
### S3 — Create web/templates/index.html
...
- Body structure:
  ...
  - `#log-drawer`: Collapsible bottom panel for trace events from WebSocket.
- Script tag loading `app.js`.

### S4 — Create web/static/app.js
...
2. **WebSocket connection**:
   ...
   - On `close`: update status to "Disconnected", attempt reconnect after 3s (max 5 attempts).
   - On `error`: log to console, update status.
3. **Task submission**:
...
### S5 — Create web/static/styles.css
...
### S6 — Update sovereignai/main.py to expose container

Add to `sovereignai/main.py`:
```python
def get_container() -> DIContainer:
    """Build and return the fully wired dependency injection container.

    This function is called by the web server on startup to obtain
    the same container instance that the core uses. It ensures the
    web server and the core share a single container registry.
    """
    return build_container()
```

This function is the ONLY entry point the web server uses to access core components. Per OR48, no direct imports of core internals.

### S7 — Create tests/test_web_server.py
...
2. `test_get_capabilities_returns_json_list`: Mock `CapabilityAPI.query_capabilities()` to return a list with one `CapabilityResponse`, assert JSON structure.
3. `test_post_task_returns_task_id`: Mock `CapabilityAPI.submit_task()` to return a task ID, assert response contains `task_id`.
4. `test_get_task_state_returns_task`: Mock `CapabilityAPI.get_task_state()` to return a `Task`, assert JSON structure.
5. `test_websocket_receives_trace_events`: Use `TestClient`'s `websocket_connect()` to connect to `/ws`, mock `AuthMiddleware.validate_token()` to return True, emit a trace event, assert the client receives it as JSON.
6. `test_websocket_rejects_invalid_token`: Connect with `?token=bad`, assert connection closed with 1008.
...
- If WebSocket test hangs for more than 5 seconds, STOP (indicates async event loop deadlock).
...
- `sovereignai/main.py` (add `get_container()`)
...
Run `/close`. Tag: `prompt-6`. Update CHANGELOG, PLANS.md, LANDMINES.md if new patterns discovered.
