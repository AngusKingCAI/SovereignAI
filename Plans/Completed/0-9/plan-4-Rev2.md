...
from sovereignai.shared.routing_engine import NoActiveProviderError
...
# Modules that are concrete core implementations — UIs and the Capability
# API may NOT import these directly. Per Finding 4 (Rev2): protocol
# modules (capability_graph, task_state_machine) are NOT in this list —
# the Capability API is allowed to import protocols (ICapabilityIndex,
# ITaskStateQuery). Only concrete implementation modules are forbidden.
...
def test_ui_directories_do_not_import_core_internals(ui_dir: str) -> None:
    ...
    Per Finding 4 (Rev2): the test now catches `from ... import`
    sub-module forms (e.g. `from sovereignai.shared import event_bus`).
...
**Files changed**:
- sovereignai/shared/types.py (extended: SessionToken, AuthError, CapabilityQuery, CapabilityResponse, RelayNotSupportedError per Finding 5)
- sovereignai/shared/auth.py (new — PBKDF2 hashing, session tokens, 8h TTL)
- sovereignai/shared/capability_api.py (new — public contract for UIs, AR7-compliant; submit_task calls state_machine.submit() per Finding 1; accepts category param per Finding 2; validates capability exists per Finding 3; imports TaskStateMachine concrete for submit per Finding 1)
...
- tests/test_capability_api.py (new — 7 tests including AR7 static-import; updated for category param + NoActiveProviderError)
- tests/test_ar7_no_core_imports_in_ui.py (new — AR7 enforcement per Finding 4: catches from...import sub-module forms; denylist excludes protocol modules)
- tests/test_relay_placeholder.py (new — 3 tests; updated for RelayNotSupportedError exception)
...
*Plan 4 — Interface Layer. Rev2. Architect draft. Part of Plans 1-4 batch — Round Table reviews alongside plan-1-Rev2, plan-2-Rev2, plan-3-Rev2, and the shared context brief. Adjudication log above records Rev1 → Rev2 changes per GR4. Q26 (Composition Root bootstrap) confirmed at this plan's /close.*
