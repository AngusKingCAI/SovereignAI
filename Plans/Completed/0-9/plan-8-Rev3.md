...
4. **SSE trace handling** (updated from Plan 6):
   ...
   - **Per Finding 22**: All trace rendering uses `textContent`, NOT `innerHTML` — prevents XSS if trace messages contain user-controlled content.
   - **Per Finding 5 (Plan 6)**: Handle `event: gap` messages by rendering a visual separator (\"--- N events dropped ---\").
   - Max 1000 traces in memory; drop oldest when limit exceeded.
   - SSE auto-reconnect handled by browser — no manual reconnect logic needed.
   - **Per Finding 16 (Plan 6)**: On reconnect, server replays from `Last-Event-ID` — client may receive duplicate events. Deduplicate by `sequence` field.
