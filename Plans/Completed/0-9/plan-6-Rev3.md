...
7. **SSE endpoint** `GET /api/traces/stream`:
   ...
   - **Per Finding 5/16**: Each event includes `id: <monotonic_seq>` field. On reconnect, read `Last-Event-ID` header and replay from TraceEmitter's ring buffer (up to 1000 events).
   - Subscribe to `TraceEmitter` events by registering a callback that formats each event as SSE data (`id: {seq}\ndata: <json>\n\n`).
