...
- In existing lifespan() in web/main.py (lines 44-53): add await event_bus.start() before yield; add await event_bus.stop() after yield. Do NOT create a new lifespan function — merge into the existing one.
...
- If publish() called before start(): raise RuntimeError('EventBus not started. Call start() before publishing.'). No silent no-op or buffering.
...
- On QueueFull: drop newest incoming event (preserve oldest), increment error counter (counted toward handler's circuit breaker), and emit queue.dropped trace event. Rationale: older events are more likely to be dependencies of future events; preserving causal order. Note: this reverses Rev 3's drop-oldest policy; causal order preservation is the priority.
...
- Rename trace.deprecated_eventType to trace.deprecated_event_type for snake_case consistency.
...
- test_async_delivery.py: FIFO order, circuit breaker trigger, sync publish same-thread (create_task), sync publish cross-thread (run_coroutine_threadsafe), queue_full_drops_newest, test_librarian_never_drops_events, test_drain_task_crash_recovery, test_publish_before_start_raises.
