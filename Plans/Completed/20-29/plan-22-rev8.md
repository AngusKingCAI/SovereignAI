...
Open questions resolved: Q-22.1 (ReActLoopFactory as Protocol), Q-22.2 (UUID session keys)
...
- Method: emit_event(event_name: str, payload: dict) -> None. Prefixes event_name with "trace." ONLY if event_name does not already start with "trace.". Converts to TraceEvent and calls inner.emit(component='trace', level=TraceLevel.INFO, message=...). Existing TraceEmitter signature (component, level, message) is unchanged.
- If EventBus is not started, falls back to inner.emit() directly (does not raise RuntimeError). Once EventBus is started, publishes via EventBus for async routing.
...
- Methods: run(task_description: str, tools: list[SkillManifest], session: SkillSession, context: dict | None = None) -> AgentResult, step(history: list[Turn]) -> ToolCall | FinalAnswer.
...
- Emits trace event per step via injected TraceEmitterWrapper. Callers use unprefixed names: "react.step_completed", "react.tool_called", etc. emit_event() handles prefixing.
...
- close(tool_id) removes the entry from dict AND calls runner.close(). Returns without error if tool_id not found (idempotent).
- close_all() iterates a COPY of registered items, calls close(tool_id) for each, then clears the dict.
...
- After 3 consecutive validation failures, raise StructuredOutputExhaustedError containing the last raw LLM response and all failure reasons. Emits trace event via injected TraceEmitterWrapper before raising. ReActLoop catches this and produces AgentErrorObservation propagated to caller, not a crash.
...
- On run() completion (success or error): call session_registry.close(self._session_key) to remove only this loop's entry.
- On StructuredOutputExhaustedError: (1) close tool connections via ToolSessionRegistry.close_all(), (2) drain _pending_tool_calls queue (discard all items), (3) emit AgentErrorObservation and return. ReActLoop emits trace.react.structured_output_exhausted via injected TraceEmitterWrapper before cleanup.
...
- Define ReActLoopFactory as Protocol class (not type alias):
  ...
  ```
- Register: container.register_factory(ReActLoopFactory, lambda: ReActLoop(...)) where ... are injected dependencies from container.
- Retrieve: loop = container.retrieve(ReActLoopFactory)  # Returns ReActLoop instance directly — no trailing ().
