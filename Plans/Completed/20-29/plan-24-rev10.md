...
- execute_task() sequence: (1) ctx = self.build_context() — may return None (degraded). (2) worker = container.retrieve(ReActLoopFactory). (3) graph_memory = container.retrieve(TaskGraphCache). (4) result = await worker.run(task_description, tools, session, context=ctx, memory=graph_memory). (5) Log a warning if ctx is None: 'Symbol context unavailable (degraded mode), proceeding without symbol ranking'. The ReActLoop contract handles ctx=None per Plan 22 S1.
...
- Latency test: test_symbol_map_latency_budget.py. Methodology: warmup run (discarded), then measure 3-run median of SymbolMap(project_root=test_path) construction+auto-index time. Budget: median ≤2000ms. Mark as @pytest.mark.slow; run on dedicated CI runner. If median exceeds budget, failure message includes all three timings to distinguish startup noise from regression.
...
- Add close() method: closes SQLite connection. Idempotent. Called by DepartmentManager.execute_task() in finally block to ensure connection cleanup even on exception.
