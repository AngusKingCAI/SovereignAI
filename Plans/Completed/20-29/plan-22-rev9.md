...
Open questions resolved: Q-22.1 (ReActLoopFactory as Protocol), Q-22.2 (UUID session keys), Q-22.3 (TraceEmitterWrapper level parameter), Q-22.4 (ISkillRunner singleton close safety)
...
- Method: emit_event(event_name: str, payload: dict, level: TraceLevel = TraceLevel.INFO) -> None. Prefixes event_name with "trace." ONLY if event_name does not already start with "trace." (case-insensitive: check event_name.lower()). Converts to TraceEvent and calls inner.emit(component='trace', level=level, message=...). Existing TraceEmitter signature (component, level, message) is unchanged.
- Check event_bus.is_started property (exposed by EventBus). If False (pre-start OR post-stop), route to inner.emit() directly. If True, publish via EventBus for async routing.
...
- Methods: run(task_description: str, tools: list[SkillManifest], session: SkillSession, context: dict | None = None, memory: GraphMemoryBackend | None = None) -> AgentResult, step(history: list[Turn]) -> ToolCall | FinalAnswer.
...
- The memory parameter receives GraphMemoryBackend from CodingManager (Plan 24 S2). If memory is not None, include graph context in the system prompt. If None, omit.
...
- close_all() iterates a COPY of registered items, calls runner.close() for each, then clears the dict. Reserved for app shutdown only.
...
- If memory parameter is provided, append "[Graph Context]" section with relevant graph entities.
...
- On StructuredOutputExhaustedError: (1) call session_registry.close(self._session_key) to remove this loop's entry, (2) drain _pending_tool_calls queue (discard all items), (3) emit AgentErrorObservation and return. Do NOT call close_all(). ReActLoop emits trace.react.structured_output_exhausted via injected TraceEmitterWrapper before cleanup (level=TraceLevel.CRITICAL).
...
- test_trace_emitter_wrapper.py: verifies emit_event prefixes correctly, no double-prefix, fallback when EventBus not started, level parameter works.
- test_react_loop_factory_protocol_registration: verifies Protocol class works as DIContainer key.
