...
- In existing lifespan() in web/main.py (lines 44-53): wrap event_bus lifecycle in try/finally. Exact order:
  ```
  ...
  ```
- This ensures event_bus.start() failure does not leak container resources, and shutdown always runs in reverse order.
...
- CRITICAL EVENT EXEMPTION: Events with trace_level=CRITICAL or event_type ending in .error or .completed are exempt from drop-newest. If the queue is full and the incoming event is critical, block until queue drains rather than drop. This prevents silent loss of terminal state events.
