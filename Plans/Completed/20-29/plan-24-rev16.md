...
Rev 16 patches P24-P (MEDIUM): PageRank O(N+E) blocks event loop when called synchronously from `build_context()`. Fixed by specifying `asyncio.to_thread` wrapping for heavy SymbolMap operations.
...
- Abstract: `execute_task(task) -> Deliverable`, `async build_context(task) -> dict | None`. Catches `SymbolMapUnavailableError`: emit `trace.department.context_build_failed`, return None.
...
- `build_context()` override (async): retrieve SymbolMap via `self._container.retrieve(SymbolMap)`, call `symbol_map.index(self._project_root)` via `await asyncio.to_thread(symbol_map.index, self._project_root)` (P24-P — avoids blocking event loop), return dict. If `SymbolMapUnavailableError`: return None.
- **`execute_task()` fixed (P24-A, P24-B, P24-L, N3, N4)**:
  ```python
  async def execute_task(self, task):
      ctx = await self.build_context(task)  # P24-O: pass task; P24-P: await async build_context
      if ctx is None:
...
- Tree-sitter extracts definitions/references. Hand-rolled PageRank ranks symbols by relevance. PageRank is O(N+E); `index()` and `query()` MUST be wrapped in `asyncio.to_thread()` by callers to avoid blocking the event loop (P24-P). `SymbolMap` methods themselves are synchronous; the async wrapping is the caller's responsibility (see S2 `build_context()`).
