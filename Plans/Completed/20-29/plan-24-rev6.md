...
- Test both paths: test_symbol_map_tree_sitter.py and test_symbol_map_degraded.py (verifies health_check returns DEGRADED when tree-sitter missing). Add test_symbol_map_latency_budget.py: asserts re-index of 100-file project completes within 2000ms. If test fails on Windows, that is a real performance regression — do NOT pre-authorize skip. Fix the regression or adjust the budget with justification.
...
- Register GraphMemoryBackend as one pluggable memory backend option via existing CapabilityGraph (sovereignai/shared/capability_graph.py). Register with ComponentManifest. Does not change existing default backend selection.
- Register SymbolMapFactory callable in DIContainer via register_factory(SymbolMapFactory, lambda: SymbolMap(...)). DepartmentManager calls container.retrieve(SymbolMapFactory)(project_root) inside each execute_task() invocation.
...
- test_department_full_cycle.py: wires mock DepartmentManager -> ReActLoop -> SkillRunner (file_search) -> EventBus -> Librarian. Verifies single task completes, emits structured events, Librarian stores at least one Observation. Precondition: runs only after Plans 21-24 merged. Fail hard on import failure — any import failure at this stage is a real regression. Use pytest.importorskip() only for optional dependencies, not for core plan imports.
