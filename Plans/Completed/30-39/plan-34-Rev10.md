...
S3.4: Merge strategy: entity dedup by name+type match. On collision: store both candidate UUIDs in `merge_conflicts` table; **newer timestamp wins for entity attributes**; both UUIDs preserved. **Canonical entity**: the UUID with the newer timestamp becomes the canonical record. Existing/new relations re-attached atomically within merge transaction: `UPDATE relations SET src_id=canonical WHERE src_id=non_canonical; UPDATE relations SET dst_id=canonical WHERE dst_id=non_canonical`. Log re-linked count. **Repeat-merge behavior**: if same name+type collision seen again, existing conflict record is updated (newer candidate wins); no duplicate conflict created. **Operator resolution** via `/api/orchestrator/memory/conflicts` — v1: `resolution_status = "unresolved"` always (no resolution API until future plan).
...
S3.8: Benchmark test: `pytest app/web/tests/test_graph_memory_query_latency.py -v` — `@pytest.mark.benchmark`; bulk insert 100k entities/500k relations; realistic queries; p99 ≤ 500ms. Per-PR: `@pytest.mark.small_scale` (10k entities). CI: small_scale default; benchmark only with `RUN_GRAPH_BENCHMARK=1`. **Benchmark gate**: `RUN_GRAPH_BENCHMARK=1 pytest ... --benchmark-only` produces report with data size, host class, p50/p95/p99. 10k test per-PR; quarterly benchmark validates 100k scale.
