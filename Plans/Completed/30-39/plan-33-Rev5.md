...
- **S4.3**: Documented v1 constraint: "v1 supports max 1 critical shutdown hook. Force-kill budget assumes 1 hook × 15s. Additional critical hooks require force-kill recalculation." Added AR check `test_max_shutdown_hooks.py` (F-2).
- **S4.5**: Sentinel path uses `platformdirs.user_cache_dir("sovereignai")`. `os.makedirs(path, exist_ok=True)` before write. Cleanup uses PID liveness check (`os.kill(pid, 0)`) — only deletes sentinels for non-running PIDs (F-60, F-15).
- **S3.2**: Health cache TTL aligned with poll interval: 5s TTL, 5s poll. Field name standardized to `health_cache_age_ms` (F-R4-1, F-59).
- **S1.4**: Documented exception rationale: "EventBus is critical because all observability, messaging, and SSE infrastructure depends on it. Degraded start without EventBus is not supported in v1." (F-R4-3).
