...
## S1 — Root-level packages + core registries + shared quant priority
...
- S1.8: Create sovereignai/shared/quant_priority.py — `QUANT_PRIORITY: list[str] = ["q4_K_M", "q4_K_S", "q5_K_M", "q5_K_S", "q6_K", "q8_0"]` (single source of truth for quant selection order, used by HF download and llama.cpp glob fallback). Add `select_best_quant(available: list[str]) -> str | None` returning highest-priority match or None.
- S1.9: Run /verify on each. Add tests/test_database_registry.py, tests/test_service_registry.py, tests/test_quant_priority.py covering register/retrieve/find_model/select_best_quant.

## S2 — HuggingFace database provider
- S2.1: Create databases/hf_database/ package: databases/hf_database/__init__.py empty.
- S2.2: Create databases/hf_database/provider.py — HFDatabaseProvider implementing DatabaseProvider. Constructor: trace, cache_dir: Path. __init__ must not perform I/O (per OR68). Methods call huggingface_hub library lazily. Import NoCompatibleQuantError from databases.base, QUANT_PRIORITY and select_best_quant from sovereignai.shared.quant_priority.
...
- S2.4: download_model(model_id): enumerate GGUF files in repo. Use `select_best_quant(available_gguf_filenames)` from sovereignai.shared.quant_priority. If no GGUF: emit ERROR trace with detail=repo_id, raise NoCompatibleQuantError(repo_id). `hf_hub_download(repo_id=model_id, filename=<selected>, local_dir=str(Path.home() / ".sovereignai" / "models" / org / name), local_dir_use_symlinks=False)`. Note: use_symlinks=False trades disk for re-download speed; document trade-off in code comment; defer env var configurability to post-Plan-19. Wrap hf_hub_download in try/except; on exception: `shutil.rmtree(local_dir, ignore_errors=True)` to clean partial files, then re-raise. Persist model_info.json with schema `{ "model_id": str, "filename": str, "quant": str, "downloaded_at": ISO8601 }` via atomic write (tempfile.NamedTemporaryFile + os.replace per OR50). Atomic metadata write per OR50. Emit trace on start/complete/failure.
...
- S2.8: Run /verify. Add tests/test_hf_database.py with hf_hub mocked. Assert __init__ performs no I/O. Test quant fallback chain via select_best_quant (including q6_K). Test NoCompatibleQuantError on empty repo. Test local_dir uses Path.home() not literal "~". Test model_info.json persists with correct schema. Test partial-file cleanup on download failure. Test model_info.json corrupt JSON → load_model falls back to glob (cross-plan test in P19).
