...
**Methods**:
- `negotiate() -> NegotiationResult`: Iterate all registered components. For each, check `manifest.core` (default `False` if absent). Core components → `_check_core_compatibility()` (strict, fatal on mismatch). Plugins → `_check_plugin_compatibility()` (lenient, disable on mismatch). **Per Rev6 F4 + Rev7**: after EACH compatibility check (both success AND failure), call `self._matrix.record(component_a, version_a, content_hash_a, component_b, version_b, content_hash_b, status)` where status is "compatible" or "incompatible". **Per Rev6 F11**: accumulate writes in memory during negotiation; call `self._matrix.flush()` once at the end of `negotiate()` to batch-write (avoids write-storm). **Per Rev7 Claude finding**: record failures too, not just successes — the audit trail must show which combinations were tested and rejected.
...
**Methods**:
- `record(component_a, version_a, content_hash_a, component_b, version_b, content_hash_b, status)`: Add entry, atomic-write to `.json`, then validate the loaded JSON before copying to `.backup` (per Rev5 F11 — prevents backing up corrupt data). If validation fails, keep the existing backup.
...
def remove(self, component_type: type) -> None:
    ...
    The EventBus must support subscriber tracking — subscribe() accepts an optional
    subscriber_id param, and unsubscribe_all(subscriber_id) removes all handlers.
    """
    with self._lock:
        instance = self._singletons.pop(component_type, None)
    # F5: unsubscribe all event handlers registered by this component instance.
    # Per Rev7 Claude finding: use id(instance) not component_type as subscriber_id.
    # Per Rev8 Kimi finding: components must register subscriptions with subscriber_id=component_id
    # (the string from the manifest). container.remove() receives component_id and passes it.
    # The EventBus subscribe() API: bus.subscribe(channel, handler, subscriber_id=component_id).
    # The EventBus unsubscribe_all(subscriber_id=component_id) removes all matching handlers.
    # subscriber_id consistency: all component-initiated subscribe() calls use their manifest component_id.
    if instance is not None and hasattr(self, '_event_bus') and self._event_bus is not None:
        # Use component_id if available on the instance, else fall back to id(instance)
