...
- web/main.py: add `GET /api/traces/history` returning `trace.recent_events()` as list[TraceEventDTO]; add `GET /api/traces/stream` SSE endpoint that calls `trace.subscribe_callback()` to receive live events and yields TraceEventDTO JSON frames to the client. Generator polls `await request.is_disconnected()` inside the loop; on disconnect, calls the returned unsubscribe function and breaks. (Standard FastAPI/Starlette pattern.)
...
- S3.5: Run /verify after each edit. Add tests/test_logs_panel.py covering: GET /api/traces/history returns 200 with list; GET /api/traces/stream returns 200 SSE; SSE subscription receives live events (verify subscribe_callback wired); filters honored client-side; unsubscribe called on client disconnect.
