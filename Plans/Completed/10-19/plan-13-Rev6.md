...
def register(self, component_id, manifest, instance) -> None:
    ...
    if self._dev_mode:
        ...
        passed = runner.check(
            ...
            is_first_party=str(component_id).startswith("sovereignai.") or manifest_path.is_relative_to(Path("adapters/internal")) or manifest_path.is_relative_to(Path("skills/official")),  # Per Rev5 F2
            instance=instance,
        )
        if not passed:
