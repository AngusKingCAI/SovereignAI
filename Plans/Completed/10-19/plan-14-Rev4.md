...
- **OR65** (revised): The Teacher worker gracefully degrades if GPU/dependencies are unavailable. Heavy ML imports are deferred inside method bodies. The `health_check()` method imports the top-level modules (`torch`, `peft`, etc.) inside a `try/except` block (not just `find_spec`) — this catches both `ImportError` (missing package) and `OSError` (missing CUDA DLLs). Source: Plan 14 Rev3 (N17).
...
(Revised per Rev3 N3 — GPU lock scoped to in-process; N17 — health_check imports modules, not just find_spec.)
...
class TeacherWorker:
    ...
    def health_check(self) -> bool:
        ...
        Per Rev3 N17: imports the top-level modules inside try/except (not just find_spec).
        This catches both ImportError (missing package) and OSError (missing CUDA DLLs).
        """
        required = ["torch", "peft", "transformers", "trl", "bitsandbytes", "accelerate", "datasets"]
        for pkg in required:
            try:
                # Import the top-level module — catches broken installs that find_spec misses
                importlib.import_module(pkg)
            except (ImportError, OSError) as e:
                self._trace.emit(
                    component="teacher",
                    level=TraceLevel.WARN,
                    message=f"QLoRA dependency '{pkg}' unavailable ({type(e).__name__}): {e}. Install with: pip install sovereignai[education]",
                )
                return False
