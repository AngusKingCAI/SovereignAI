...
- S2.4: download_model(model_id): `hf_hub_download(repo_id=model_id, filename=<gguf file from repo>, local_dir=~/.sovereignai/models/{org}/{name}/)`. Enumerate GGUF files in repo, select first q4_K_M variant. Atomic metadata write per OR50 (temp + os.replace). Emit trace on start/complete/failure.
...
- S2.8: Run /verify. Add tests/test_hf_database.py with hf_hub mocked. Assert __init__ performs no I/O.

## S3 — Ollama service provider
- S3.1: Create services/ollama_service/ package: services/ollama_service/__init__.py empty.
- S3.2: Create services/ollama_service/provider.py — OllamaServiceProvider implementing ServiceProvider. Constructor: trace, port: int = 11434. __init__ must not perform I/O (per OR68).
- S3.3: start(): check `ollama` on PATH via shutil.which. If absent, raise typed ServiceNotFoundError. Spawn `ollama serve` as subprocess via `subprocess.Popen`, capture pid. Poll http://localhost:{port}/api/version until 200 or 10s timeout. On timeout: `proc.terminate()`, wait 2s, `proc.kill()` fallback, emit trace, raise ServiceStartError.
...
- S4.1: Edit sovereignai/main.py build_container() — instantiate DatabaseRegistry and ServiceRegistry, register HFDatabaseProvider and OllamaServiceProvider, add to DI container. Guard with `if not _test_mode` (existing pattern from prompt-15.1). Providers' __init__ does no I/O, so test-mode guard at single call site is sufficient (per OR68).
