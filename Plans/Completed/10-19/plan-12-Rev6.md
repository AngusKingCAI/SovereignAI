...
**Methods**:
- `negotiate() -> NegotiationResult`: Iterate all registered components. For each, check `manifest.core` (default `False` if absent). Core components → `_check_core_compatibility()` (strict, fatal on mismatch). Plugins → `_check_plugin_compatibility()` (lenient, disable on mismatch). **Per Rev6 F4**: after each successful compatibility check, call `self._matrix.record(component_a, version_a, content_hash_a, component_b, version_b, content_hash_b, status)` to persist the result. **Per Rev6 F11**: accumulate writes in memory during negotiation; call `self._matrix.flush()` once at the end to batch-write (avoids write-storm on startup with many plugins).
...
def remove(self, component_type: type) -> None:
    ...
    # F5: unsubscribe all event handlers registered by this component
    if instance is not None and hasattr(self, '_event_bus') and self._event_bus is not None:
        self._event_bus.unsubscribe_all(subscriber_id=str(component_type))
        self._trace.emit(
            component="container",
