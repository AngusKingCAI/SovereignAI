...
def test_librarian_route_finds_providers() -> None:
    ...
    graph = CapabilityGraph(trace=trace)
    librarian = Librarian(capability_graph=graph, trace=trace)
    manifest = ComponentManifest(component_id=ComponentId('episodic_memory'), version='0.1.0', provides=(CapabilityDeclaration(category=CapabilityCategory.MEMORY, name='episodic', version='1.0.0', priority=100),), requires=(), author='system', content_hash='sha256:placeholder')
...
def test_librarian_merge_results_episodic_dedupes_by_id() -> None:
    ...
    graph = CapabilityGraph(trace=trace)
    librarian = Librarian(capability_graph=graph, trace=trace)
    results = [{'id': '1', 'timestamp': 2.0}, {'id': '2', 'timestamp': 1.0}, {'id': '1', 'timestamp': 3.0}]
