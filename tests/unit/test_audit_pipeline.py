# tests/unit/test_audit_pipeline.py
import json
import pytest
...
def test_log_decision_writes_valid_jsonl(tmp_path):
    ...
    from scripts.enforcement.pre_tool_pdp import log_decision
    
    # Override AUDIT_LOG to use temp directory
    import scripts.enforcement.pre_tool_pdp as pdp_module
...
def test_weekly_report_aggregates_correctly(tmp_path):
    ...
    from scripts.audit.weekly_review_report import REPORT_DIR, AUDIT_LOG
    
    # Override paths
    import scripts.audit.weekly_review_report as report_module
    ...
    try:
        ...
        violations = [
            {"timestamp": datetime.utcnow().isoformat() + "Z", "tool": "exec", "input_summary": "rm -rf", "decision": "deny", "rule_id": "SHARED-S01", "reason": "destructive"},
            {"timestamp": datetime.utcnow().isoformat() + "Z", "tool": "exec", "input_summary": "rm -rf", "decision": "deny", "rule_id": "SHARED-S01", "reason": "destructive"},
            {"timestamp": datetime.utcnow().isoformat() + "Z", "tool": "write", "input_summary": "file.txt", "decision": "allow", "rule_id": "SHARED-001", "reason": ""},
        ]
        ...
        assert "SHARED-S01: 2 denials" in report_content
