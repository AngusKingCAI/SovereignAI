...
def test_submit_sets_received(machine: TaskStateMachine) -> None:
    task = Task(task_id=uuid4(), capability=CapabilityDeclaration(category=CapabilityCategory.TOOL, name='test_tool', version='1.0.0'), payload='{}', submitted_at=datetime.now(UTC))
    machine.submit(task)
...
def test_list_tasks_returns_insertion_order(machine: TaskStateMachine) -> None:
    task1 = Task(task_id=uuid4(), capability=CapabilityDeclaration(category=CapabilityCategory.TOOL, name='test_tool', version='1.0.0'), payload='{}', submitted_at=datetime.now(UTC))
    task2 = Task(task_id=uuid4(), capability=CapabilityDeclaration(category=CapabilityCategory.TOOL, name='test_tool', version='1.0.0'), payload='{}', submitted_at=datetime.now(UTC))
    task3 = Task(task_id=uuid4(), capability=CapabilityDeclaration(category=CapabilityCategory.TOOL, name='test_tool', version='1.0.0'), payload='{}', submitted_at=datetime.now(UTC))
...
def test_submit_with_valid_dag_passes(machine: TaskStateMachine) -> None:
    task = Task(task_id=uuid4(), capability=CapabilityDeclaration(category=CapabilityCategory.TOOL, name='test_tool', version='1.0.0'), payload='{}', submitted_at=datetime.now(UTC))
    dag = DAGSpec(nodes=('node1', 'node2'), edges=(('node1', 'node2'),), input_types={'node2': 'type1'}, output_types={'node1': 'type1'})
    machine.submit(task, dag=dag)
    assert machine.get_state(task.task_id) == TaskState.RECEIVED

def test_submit_with_invalid_dag_raises(machine: TaskStateMachine) -> None:
    task = Task(task_id=uuid4(), capability=CapabilityDeclaration(category=CapabilityCategory.TOOL, name='test_tool', version='1.0.0'), payload='{}', submitted_at=datetime.now(UTC))
    dag = DAGSpec(nodes=('node1', 'node2'), edges=(('node1', 'node2'), ('node2', 'node1')), input_types={}, output_types={})
