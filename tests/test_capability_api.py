...
def test_query_capabilities_valid_token_returns_providers(api: CapabilityAPI, auth: AuthMiddleware, capability_index: CapabilityGraph) -> None:
    ...
    capability_index.register(ComponentManifest(component_id=ComponentId('TestProvider'), version='1.0.0', provides=(CapabilityDeclaration(category=CapabilityCategory.TOOL, name='websearch', version='1.0.0', priority=0),), requires=(), author='test', content_hash='abc123'))
    from sovereignai.shared.types import CapabilityQuery
    response = api.query_capabilities(token, CapabilityQuery(category=CapabilityCategory.TOOL, name='websearch'))
...
def test_query_capabilities_invalid_token_raises(api: CapabilityAPI) -> None:
    ...
    with pytest.raises(AuthError):
        api.query_capabilities('invalid_token', CapabilityQuery(category=CapabilityCategory.TOOL, name='websearch'))

def test_submit_task_valid_token_returns_uuid(api: CapabilityAPI, auth: AuthMiddleware, capability_index: CapabilityGraph) -> None:
    auth.register_user('testuser', 'password')
    token = auth.login('testuser', 'password').token
    capability_index.register(ComponentManifest(component_id=ComponentId('TestProvider'), version='1.0.0', provides=(CapabilityDeclaration(category=CapabilityCategory.TOOL, name='websearch', version='1.0.0', priority=0),), requires=(), author='test', content_hash='abc123'))
...
def test_get_task_state_valid_token_returns_none_for_unknown(api: CapabilityAPI, auth: AuthMiddleware) -> None:
    auth.register_user('testuser', 'password')
    token = auth.login('testuser', 'password').token
...
def test_submit_task_with_category_param(api: CapabilityAPI, auth: AuthMiddleware, capability_index: CapabilityGraph) -> None:
    auth.register_user('testuser', 'password')
    token = auth.login('testuser', 'password').token
    capability_index.register(ComponentManifest(component_id=ComponentId('ModelProvider'), version='1.0.0', provides=(CapabilityDeclaration(category=CapabilityCategory.MODEL_INFERENCE, name='text_generation', version='1.0.0', priority=0),), requires=(), author='test', content_hash='abc123'))
    task_id = api.submit_task(token, CapabilityCategory.MODEL_INFERENCE, 'text_generation', '{"prompt": "hello"}')
    assert isinstance(task_id, UUID)

def test_submit_task_then_get_state_returns_received(api: CapabilityAPI, auth: AuthMiddleware, capability_index: CapabilityGraph, task_state_machine: TaskStateMachine) -> None:
    auth.register_user('testuser', 'password')
    token = auth.login('testuser', 'password').token
    capability_index.register(ComponentManifest(component_id=ComponentId('TestProvider'), version='1.0.0', provides=(CapabilityDeclaration(category=CapabilityCategory.TOOL, name='websearch', version='1.0.0', priority=0),), requires=(), author='test', content_hash='abc123'))
    task_id = api.submit_task(token, CapabilityCategory.TOOL, 'websearch', '{"query": "test"}')
