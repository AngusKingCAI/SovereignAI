...
def test_register_first_run(client, container):
...
def test_register_after_user_exists(client, container):
...
def test_protected_endpoint_without_cookie(client, container):
...
def test_protected_endpoint_with_cookie(client, container):
    ...
    response = client.get("/api/capabilities", cookies={"session_id": session_cookie})
    
    assert response.status_code == 200


def test_sse_auth_required(client, container):
