...
S0.4 — Verify all existing ComponentManifest instantiations use keyword arguments: `git grep -n "ComponentManifest(" . -- ':!vendor/*' ':!*.pyc'`. If any use positional arguments beyond the first field, convert to kwargs before adding metadata field. Search the ENTIRE repository, not just sovereignai/ — tests, skills, adapters, web, scripts, and fixtures must all be checked. Add scripts/check_component_manifest_kwargs.py: AST-based scan that rejects any Call node to ComponentManifest with more than one positional argument. Wire into pre-commit config.
...
- Concrete close() implementations must be idempotent — safe to call more than once. ToolSessionRegistry.close_all() (Plan 22 S1.1) invokes close() once during app shutdown; idempotency is a defensive requirement for this single call path.
...
- _module_cache is process-scoped; ConcreteSkillRunner is singleton per S8. No refcount needed.
- run() resolves skill_id via CapabilityGraph.find_providers(CapabilityCategory.SKILL, skill_id) where skill_id is the component_id. If multiple providers match, use the first (highest priority per CapabilityDeclaration.priority). If none match, raise SkillNotFoundError.
- close() clears _module_cache if not empty. Idempotent: if _module_cache is already empty, return without error.

### S2 — Create SkillManifest dataclass in sovereignai/skills/manifest.py
- Fields: id, name, version, capabilities list, execution config (runner, timeout), dependencies: list[str], memory_integration_hints.
...
- Each skill's manifest.toml gains optional [dag] section with dag_dependencies = []. Parsed by from_toml() as list[str]. DAG semantics deferred to a future executor plan; from_toml() parses only, no cross-reference validation.
- to_component_manifest() method returns ComponentManifest with: component_id=self.id, display_name=self.name, version=self.version, category=CapabilityCategory.SKILL, capabilities=self.capabilities. Map dependencies → metadata['dependencies'], memory_integration_hints → metadata['memory_hints']. Add metadata: dict[str, Any] = field(default_factory=dict) as LAST field to ComponentManifest in sovereignai/shared/types.py. Only valid after S0.4 verification confirms all existing instantiations use keyword arguments.
- Document: metadata values are validated by consumers at read time, not by ComponentManifest.
...
- Register discovered capabilities in existing CapabilityGraph (sovereignai/shared/capability_graph.py) via to_component_manifest(). Use CapabilityGraph.register(manifest=component_manifest, instance=None) — instance is deferred to ConcreteSkillRunner.run(). Manifest-only registration is supported by existing register() (guarded by "if instance is not None").
- Maintain internal _skill_index: dict mapping skill_id → (CapabilityCategory.SKILL, skill_id) for lookup by ConcreteSkillRunner. skill_id IS the capability name used in find_providers().
...
- Register CapabilityGraph as singleton (already exists in repo; ensure it is registered if not already). Use try/except KeyError with narrow scope: try container.retrieve(CapabilityGraph); except KeyError: container.register_singleton(CapabilityGraph, CapabilityGraph(trace=container.retrieve(TraceEmitter))). Document: _instances access is a temporary measure; add is_registered() in future refactor plan.
...
- test_skill_manifest.py: manifest parsing, no-import guarantee, sys.modules unchanged, dag_parse_simple, dag_parse_missing_optional, test_to_component_manifest_mapping (verifies dependencies and memory_hints mapped to metadata).
...
- test_skill_discovery.py: auto-discovery, explicit disable, dangling dependency warning logged, test_capability_graph_registration (verifies manifest-only registration with instance=None).
...
- test_capability_category_skill_exists: verifies SKILL enum member exists and is usable.
- test_capability_category_json_serializable: verifies json.dumps(CapabilityCategory.SKILL) produces valid string equal to "SKILL" (not "1" or other IntEnum artifact). If CapabilityCategory is IntEnum, ensure __str__ returns name or override serialization.
