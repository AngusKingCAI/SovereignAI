...
- **S1.2**: Cookie jar path uses `platformdirs.user_cache_dir("sovereignai")` instead of hardcoded `~/.cache/sovereignai/`. `os.makedirs(path, exist_ok=True)` before write (F-15).
- **S4.4**: Sentinel path uses `platformdirs.user_cache_dir("sovereignai")` instead of hardcoded `~/.cache/sovereignai/`. Matches Plan 33 writer path on all platforms (F-15).
- **S4.4**: Sentinel content includes timestamp. TUI checks both PID match AND timestamp freshness (< 5 min) to mitigate PID reuse (F-3).
- **S3.12**: `not_ready_timeout` explicitly classified as terminal error (F-58 cross-ref).
