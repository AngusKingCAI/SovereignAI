...
**Other methods** (unchanged): `query`, `delete`, `get_last_task_states()`.
...
def _on_trace_emitted(event: TraceEvent) -> None:
    ...
    try:
        trace_backend.store(
            ...
            metadata={},  # task_id/task_state populated by query if needed
        )
    except Exception:
        # Never let trace persistence break the emitter
        pass
...
def run_crash_recovery(container, trace) -> None:
    ...
    try:
        if os.path.exists(marker_path):
            # Previous shutdown was clean — delete marker and skip recovery
