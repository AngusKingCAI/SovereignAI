...
- **S5.1**: Bootstrap replaced setup-token model. Auto-provisioning removed. First run prints single-use setup token to stderr. First login requires token. Token expires after first successful login or 24h. Eliminates unauthenticated admin takeover (F-1, F-17).
- **S5.1**: Added CLI unlock command: `python -m sovereignai.auth.unlock <username>` — resets lockout state in SQLite audit DB. Requires shell access (F-16, F-R4-4).
- **S5.1**: Bootstrap exempt from hard lockout. Lockout only applies when `users.json` exists (F-13).
- **S5.1**: `.gitignore` path uses `platformdirs.user_config_dir("sovereignai") / ".gitignore"` (F-14).
- **S3.1**: Idempotency cache scoped to `(user_session_id, idempotency_key, endpoint_path, payload_hash)`. Cross-user replay prevented. Payload mismatch = new request (F-4).
- **S3.1**: Idempotency cache explicitly assigned to audit DB (F-18).
...
S3.1: POST `/api/orchestrator/message` — accepts `MessageRequest`, returns `OrchestratorResponse`. Supports `Idempotency-Key: UUID4` header; server stores `(user_session_id, idempotency_key, endpoint_path, payload_hash → response)` in audit DB SQLite with 5-min TTL. Retries with same key, same session, same payload return cached response. Key match with different payload = new request (no replay). Cross-user replay prevented by session_id scoping.
