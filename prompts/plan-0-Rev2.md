...
**Date**: 2026-06-28
**Plan file**: prompts/plan-0-Rev2.md

**Files changed**:
- AGENTS.md (added OR39)
- .devin/workflows/open.md (fixed master -> main)
- documents/project-vision-Rev5.md (fixed title + revision history)
- PLANS.md (state update)
- CHANGELOG.md (new)
- LANDMINES.md (new)
- DECISIONS.md (new)
- DEBT.md (new)
- pyproject.toml (new)
- txt/requirements.txt (new, empty)
...
- pip-audit: 0 CVEs (dev tools only, no runtime deps)
...
Append-only. L1–L23 inherited from sovereign-ai (predecessor project). New landmines for SovereignAI start at L24.
...
# sovereignignai/, web/, cli/, tui/, or phone/.
#
# Dev-only tools (pytest, ruff, mypy, etc.) go in pyproject.toml's
# [project.optional-dependencies] dev = [...] — NOT here.
#
# After any change, run: pip install -e .[dev] && pip-audit --strict
```

(File starts empty — no runtime deps yet. First entry will be added in Plan 1 when the first import lands.)
...
### S3.5 — `txt/.secrets.baseline`

This file is **generated**, not hand-written. After S3.1–S3.4 are created and committed-at-staged, install dev dependencies and generate the baseline:
...
After creating each file in S3.1–S3.4, run `/verify`. S3.5 runs `/verify` after the baseline is generated.

---

## S4 — Create Directory Structure
...
## S5 — Fix `project-vision-Rev5.md` Title and Revision History
...
1. In `documents/project-vision-Rev5.md`:
   - Line 1: change `# Founding Vision — The Unnamed Project (Rev 4)` to `# Founding Vision — The Unnamed Project (Rev 5)`
   - In the status block at the top, change "post-third-round-table" to "post-third-round-table, locked canonical revision"
   - Update the "What changed since Rev 3" line: change "Rev 4" prefix references to "Rev 5" where they describe what's locked. The simplest correct edit is to prepend: "Rev 5 is the locked canonical revision. No content changes from Rev 4 — Rev 4 received clean-pass #3 from the Round Table. This file's title and revision history are corrected to reflect Rev 5 status."
   - Append a new entry to the "Revision history" section at the bottom:
     ```
...
2. Verify the `prompt-0` row in "Completed Prompts" table is present and accurate (it will be — the row was already there from the initial bootstrap commit).
3. Verify Plan 1 is marked "🔜 Ready to draft" in the Next-5-Prompt Queue (it already is).
...
1. Stage all changes (note: AGENTS.md was already committed at S0.3, so it won't be in this commit):
   ...
   ```
   Verify the staged files match the "Files WILL Create" and "Files WILL Edit" lists below. If anything unexpected is staged, STOP.
...
2. Commit:
   ...
   - Add txt/requirements.txt (empty), txt/vulture-whitelist.txt (empty)
   - Add txt/.secrets.baseline (generated via detect-secrets scan)
   ...
   - Fix project-vision-Rev5.md title and revision history (was byte-identical to Rev4)"
   ```
...
Mark prompt-0 as complete and set Plan 1 as the active plan. Update the "Active Plan" section.
...
- Mypy: N/A (no Python code yet)
- Bandit: N/A (no Python code yet)
- pip-audit: 0 CVEs expected (dev tools only, `txt/requirements.txt` is empty so no runtime deps scanned)
- Vulture: N/A (no Python code yet)
- Detect-secrets: pass (baseline established at S3.5)
...
- `txt/.secrets.baseline` (generated)
- `logs/.gitkeep`
- `tests/.gitkeep`
- `sovereignai/.gitkeep`
- `sovereignai/orchestrator/.gitkeep`
...
- `sovereignai/librarian/.gitkeep`
- `sovereignai/adapters/.gitkeep`
- `sovereignai/adapters/external/.gitkeep`
- `sovereignai/skills/.gitkeep`
- `sovereignai/skills/user/.gitkeep`
- `sovereignai/skills/external/.gitkeep`
- `sovereignai/shared/.gitkeep`
- `web/.gitkeep`
- `cli/.gitkeep`
- `tui/.gitkeep`
- `phone/.gitkeep`
...
- `documents/project-vision-Rev5.md` (fix H1 title + append Rev 5 revision history entry at S5)
- `PLANS.md` (update state at S6 and S8)
...
- `prompts/plan-0.md` (Rev1 — superseded by this Rev2 file)
- `prompts/plan-0-brief.md` (Rev1 brief — Round Table reviews Rev2 directly, no new brief needed)

---

*Plan 0 — Prompt-0 Remediation. Rev2. Architect draft. Round Table reviews this file directly per AI_HANDOFF.md Rev2+ process.*
