...
class NoActiveProviderError(Exception):
    ...
    Per Rev7 Finding 2: this class is ALREADY DEFINED in shared/types.py
    by Plan 3 S1.1 (per Rev4 Finding 2). Do NOT re-define it here — the
    Plan 3 definition is canonical. If the Executor finds this class
    already exists in types.py when appending Plan 4's types, SKIP this
    definition and add a comment: "# NoActiveProviderError already defined
    by Plan 3 S1.1 — no action needed."
    """
