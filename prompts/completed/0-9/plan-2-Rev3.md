...
### Finding 12 — Manifest parser priority ValueError (Kimi MEDIUM)
**Severity**: MEDIUM
**Reasoning**: Rev2 S2.1 `_parse_caps` did `priority=int(entry.get("priority", 0))` with no try/except. A manifest entry with `priority = "high"` raises unhandled `ValueError` instead of the documented `ManifestParseError`.
**Action**: ACCEPTED. `_parse_caps` now wraps the `int()` conversion in try/except and raises `ManifestParseError` with the entry index and path.

### Rejected findings

- **DeepSeek** "manifest parser doesn't validate semver format" — Q8 is explicitly MVP; full versioning deferred. Basic presence check is sufficient.
- **Kimi** "ICapabilityIndex.find_providers masks which provider is best — RoutingEngine couples to lifecycle" — this is by design. The RoutingEngine is supposed to couple to the LifecycleManager (both are in Plan 3). The protocol returns all providers sorted by priority; the RoutingEngine filters by status. This is the intended architecture per A1.

### Verdict

All MEDIUM issues fixed. Plan 2 Rev3 is ready for execution.
...
### Finding 1 — `ICapabilityIndex.find_providers` return type mismatch (GPT-5.4-mini HIGH, DeepSeek HIGH, Kimi HIGH, Qwen HIGH — convergent)
**Severity**: HIGH (blocks mypy at /close)
**Reasoning**: Rev1 S3.1 Protocol declared `-> Tuple[ComponentId, CapabilityDeclaration]` (a single 2-tuple), but the concrete `CapabilityGraph.find_providers` returns `tuple(self._index.get(key, []))` which is `Tuple[Tuple[ComponentId, CapabilityDeclaration], ...]` (a collection of pairs). Mypy reports incompatible override. Plan 3's RoutingEngine iterates over the result as a collection — confirming the annotation is wrong, not the implementation.
**Action**: ACCEPTED. Protocol return type changed to `Tuple[Tuple[ComponentId, CapabilityDeclaration], ...]`.

### Finding 2 — Manifest parser doesn't validate `name`/`version` presence in capability entries (Kimi MEDIUM)
**Severity**: MEDIUM
**Reasoning**: Rev1 S2.1 `_parse_caps` checks `category` but then directly accesses `entry["name"]` and `entry["version"]` without validation. A manifest entry with valid `category` but missing `name` raises `KeyError` instead of the documented `ManifestParseError`.
**Action**: ACCEPTED. `_parse_caps` now validates all required fields and raises `ManifestParseError` with the entry index and path.

### Finding 3 — `CapabilityGraph` doesn't emit traces despite P9 claim (Qwen MEDIUM)
**Severity**: MEDIUM (vision principle compliance)
**Reasoning**: Rev1 Plan 2 header claims P9 (observability). The `CapabilityGraph` class does not receive a `TraceEmitter` and does not emit traces. P9 claim is false.
**Action**: ACCEPTED. `CapabilityGraph.__init__` now accepts a `trace: TraceEmitter` parameter. `register()` emits a DEBUG trace. The Composition Root in S5.1 passes the TraceEmitter.

### Finding 4 — `ComponentManifest.component_id` type mismatch in parser (Kimi MEDIUM)
**Severity**: MEDIUM (mypy strict)
**Reasoning**: Rev1 S1.1 types `component_id` as `ComponentId` (a `NewType`). S2.1 parser does `component_id = data.get("component_id")` (returns raw `str` from TOML) and passes it directly. Mypy reports incompatible type.
**Action**: ACCEPTED. Parser now wraps as `ComponentId(data["component_id"])`.

### Rejected findings

- **DeepSeek** "manifest parser doesn't validate semver format" — Q8 is explicitly MVP; full versioning deferred. Basic presence check (Finding 2) is sufficient. Semver regex validation deferred with Q8.
- **Qwen** "Plan 2 claims P9 but graph doesn't emit traces" — accepted as Finding 3 above.

### Verdict

All HIGH/MEDIUM issues fixed. Plan 2 Rev2 is ready for execution.
...
- sovereignai/shared/capability_graph.py (new — in-memory index, ICapabilityIndex protocol; return type fixed per Finding 1; accepts TraceEmitter per Finding 3)
...
*Plan 2 — Discovery Layer. Rev3. Architect draft. Part of Plans 1-4 batch — Round Table reviews alongside plan-1-Rev3, plan-3-Rev3, plan-4-Rev3, and the shared context brief. Adjudication logs above record Rev1 → Rev2 → Rev3 changes per GR4.*
