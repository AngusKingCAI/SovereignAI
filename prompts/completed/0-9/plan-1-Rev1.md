...
### S1.3 — Add `dependency-injector` to `txt/requirements.txt`

Append to `txt/requirements.txt` (per OR39):

```
dependency-injector>=1.2
```
...
If pip-audit reports CVEs for `dependency-injector`, STOP and report — the version pin may need adjustment.

After install, run `/verify`.
...
## S5 — Composition Root (Incremental — Plan 1 Components Only)
...
if __name__ == "__main__":
    ...
    trace.emit(
        component="main",
        level="info",  # TraceLevel.INFO — string coerces via str Enum
...
**Files changed**:
- txt/requirements.txt (added first runtime dep: dependency-injector>=1.2 per OR39)
...
- sovereignai/shared/event_bus.py (new — in-order per channel, no silent failures per P10)
- sovereignai/shared/trace_emitter.py (new — singleton observability surface, NOT a context bag per AR6)
- sovereignai/shared/container.py (new — passive typed DI registry, no auto-wiring per A8)
- sovereignai/main.py (new — incremental Composition Root, wires Plan 1 components only per A3)
...
- tests/test_di_container.py (new — 5 tests: singleton, factory, precedence, missing)
...
- DI container is passive (A8) — no @inject, no auto-wiring. main.py does all explicit instantiation.
