...
**v1 simplification**: The dispatcher does NOT use an LLM for routing. It queries the CapabilityGraph for all registered skills, filters by **simple keyword matching** (case-insensitive substring check on capability name against words in the message — per Finding 4, NOT regex), and routes to the highest-priority match. **Per Finding 18 (MiniMax C1): If no match, return a structured \"I don't know how to handle that yet.\" response — do NOT fall back to Ollama as a catch-all.** The catch-all was dangerous because it silently routed unmatched messages to the most expensive adapter, producing plausible-looking but wrong answers.
...
- `_find_matching_skill(message: str) -> Optional[ComponentId]`: Query CapabilityGraph for skills, filter by **keyword matching** (case-insensitive substring: check if any word in `message` contains the capability `name`). Return highest-priority match. **Per Finding 18: If no match, return `None` — caller returns \"I don't know how to handle that yet.\"**
...
- `probe() -> HardwareInfo`:
  ...
  - GPU/VRAM: **Per Finding 8**: Multi-platform GPU detection:
    - Windows: `subprocess.run(["wmic", "path", "win32_VideoController", "get", "Name,AdapterRAM", "--format:csv"])` — parses ALL GPUs, not just NVIDIA.
