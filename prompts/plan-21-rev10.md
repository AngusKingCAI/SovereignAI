...
S0.4 — Verify all existing ComponentManifest instantiations use keyword arguments: `git grep -n "ComponentManifest(" sovereignai/`. If any use positional arguments beyond the first field, convert to kwargs before adding metadata field.
...
- test_capability_category_json_serializable: verifies json.dumps(CapabilityCategory.SKILL) produces valid string.
