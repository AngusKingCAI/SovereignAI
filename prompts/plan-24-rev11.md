...
- DepartmentManager is stateless — all caches (SymbolMap, TaskGraphCache) are scoped per execute_task() call, not shared across calls.
...
- Abstract method: build_context(task) -> dict | None. Returns symbol context as dict for the task, or None if unavailable. Catches SymbolMapUnavailableError: emit trace.department.context_build_failed event, return None. Do not fail the task — let the Worker proceed without symbol context.
- Document: Base DepartmentManager.execute_task() passes context dict to Worker. The context dict contains only symbol-context payload; memory backends are passed via the memory parameter (see S2).

### S2 — Create CodingManager in sovereignai/managers/coding.py
- Extends DepartmentManager.
- Constructor: __init__(self, container: DIContainer, project_root: Path | None = None). Calls super().__init__(container) and stores project_root as self._project_root.
...
- execute_task() sequence with explicit try/finally (DD-24.11.1):
  ...
  async def execute_task(self, task):
      ctx = self.build_context()  # may return None (degraded)
      if ctx is None:
          log.warning('Symbol context unavailable (degraded mode), proceeding without symbol ranking')
      worker = container.retrieve(ReActLoopFactory)  # returns instance directly, no trailing ()
      ...
      try:
          ...
          graph_memory.close()  # idempotent per DD-24.11.2
      return result
  ```
- Spawns isolated ReAct Worker per task via container.retrieve(ReActLoopFactory) — retrieve returns ReActLoop instance directly (DD-24.8.2). No trailing ().
- Retrieves TaskGraphCache per task: graph_memory = container.retrieve(TaskGraphCache). Passes to Worker as memory parameter (NOT in context dict). ReActLoop.run(..., memory=graph_memory) receives it separately from prompt context.
- The ctx=None warning is logged BEFORE worker spawn (DD-24.11.3), not after worker completion.
...
- Constructor: __init__(self, project_root: Path | None = None). If project_root provided, call self.index(project_root) during initialization. If project_root is None, skip indexing — caller must call index() explicitly.
...
- Test both paths: test_symbol_map_tree_sitter.py and test_symbol_map_degraded.py (verifies health_check returns DEGRADED when tree-sitter missing).
- Latency test: test_symbol_map_latency_budget.py. Methodology: warmup run (discarded), then measure 5-run median of SymbolMap(project_root=test_path) construction+auto-index time. Budget: median ≤2000ms. Mark as @pytest.mark.slow; run on dedicated CI runner. Capture all individual timings in failure output. If median exceeds budget, failure message includes all five timings to distinguish startup noise from regression (DD-24.11.4).

### S5 — Create TaskGraphCache in sovereignai/memory/graph_backend.py
- SQLite adjacency tables: entities (id, type, attributes), relations (source, target, relation).
- Recursive CTE for traversal: query(entity_id: str, depth: int = 2) -> list[dict]. Return type explicitly list[dict] to match GraphMemory Protocol (Plan 22 S1.3).
- No new dependencies — SQLite already required (DD-21.12.1).
- Constructor: __init__(self, db_path: Path | str = ':memory:'). Default db_path is ':memory:' for per-task ephemeral storage. Document: TaskGraphCache is per-task ephemeral scratch space, NOT persistent cross-task memory. Cross-task persistent graph memory is deferred to a future plan (DEBT.md entry: "Cross-task persistent graph memory — target plan TBD").
- Fresh instance per task (registered as factory, not singleton).
- Add close() method: closes SQLite connection. Idempotent via _closed flag (DD-24.11.2):
  ```python
  def close(self) -> None:
      if self._closed:
          return
      ...
      try:
          ...
          pass  # already closed
  ```
- Called by DepartmentManager.execute_task() in finally block to ensure connection cleanup even on exception.

### S6 — Wire in main.py build_container()
- Register CodingManager in DI container.
- Register TaskGraphCache as FACTORY (not singleton) via DIContainer.register_factory(TaskGraphCache, lambda: TaskGraphCache()). Fresh instance per retrieve().
- Register SymbolMap as FACTORY (not singleton) via DIContainer.register_factory(SymbolMap, lambda: SymbolMap()). DepartmentManager calls container.retrieve(SymbolMap) to get a fresh SymbolMap instance, then calls instance.index(project_root) before querying. retrieve() returns instance directly — no trailing ().
...
- test_graph_memory.py: traversal + persistence within same instance, recursive CTE, test_task_graph_cache_close_idempotent (verifies double-close is safe via _closed flag).
- test_department_full_cycle.py: wires mock DepartmentManager -> ReActLoop -> SkillRunner (file_search) -> EventBus -> Librarian. Verifies single task completes, emits structured events, Librarian stores at least one Observation. Use runtime guard inside test body (NOT @pytest.mark.skipif): def test_department_full_cycle(): try: import sovereignai.agent.react; import sovereignai.shared.event_bus; import sovereignai.managers.coding; except ImportError: pytest.skip('Requires Plans 21-24'). Remove guard after all plans land.
