...
Open questions resolved: Q-24.1 (DIContainer factory semantics), Q-24.2 (GraphMemoryBackend wiring)
...
- Retrieves GraphMemoryBackend per task: graph_memory = container.retrieve(GraphMemoryBackend). Passes to Worker context under key 'graph_memory'.
...
- Test both paths: test_symbol_map_tree_sitter.py and test_symbol_map_degraded.py (verifies health_check returns DEGRADED when tree-sitter missing). Add test_symbol_map_latency_budget.py: measures single-shot cold-start latency of SymbolMap(project_root=...) construction + first index() call. Budget is met if time ≤2000ms. No warmup. If test fails, failure message includes timing to distinguish startup noise from regression.
