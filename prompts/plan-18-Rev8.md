...
## S4 — Models panel UI + Hardware panel UI (server-side tok/s)
WILL edit UI elements:
- web/templates/index.html: replace `<section id="panel-models">` with: filter bar (search, category dropdown, VRAM-fit checkbox, quant-level dropdown), sortable table (Org, Family, Version, Quant, Size, VRAM, Tok/s [estimated], Source), empty-state div. Replace `<section id="panel-hardware">` with: CPU gauge, RAM gauge, GPU cards (name, VRAM bar, utilization bar), Disk list.
- web/static/app.js: add `loadModelsPanel()` fetching /api/models (includes tok_s_estimated field) + /api/hardware, renders rows, applies VRAM badge class, wires column sort. Display tok/s verbatim from server with "~" prefix and "(estimated)" tooltip; render "—" when tok_s_estimated is None. No client-side tok/s computation. Add `loadHardwarePanel()` subscribing to /api/hardware/stream SSE via `eventSource.addEventListener('hardware', handler)`, updating gauges/bars at 1Hz.
- web/static/styles.css: `.models-table`, `.vram-badge-vram` (green), `.vram-badge-cpu-offload` (amber #CC8400 AA), `.vram-badge-disk` (blue), `.vram-badge-na` (grey), `.empty-state`. Drop "VRAM+RAM" badge — use "CPU-offload" instead. Add `.hw-gauge`, `.hw-bar`, `.hw-bar-fill`, `.gpu-card`, `.disk-row`.
- web/main.py: add `GET /api/models` accepting query params (search, category, vram_fit, quant_level) -> list[ModelEntryDTO]. Server computes tok_s_estimated via tok_sampler.estimate_tok_s() using capability_api.sample_hardware() (NOT direct HardwareProbe import — OR69). Add `GET /api/hardware` calling capability_api.sample_hardware(); add `GET /api/hardware/stream` SSE returning `StreamingResponse(_hardware_event_stream(), media_type="text/event-stream")` where `_hardware_event_stream()` yields `event: hardware\ndata: {json.dumps(snapshot_dto.model_dump(mode='json'))}\n\n` frames from capability_api.stream_hardware().
...
- S4.2: Add tests/test_models_panel.py covering: GET /api/models returns 200 with tok_s_estimated populated; VRAM badge logic (VRAM if vram_required_mb ≤ max(gpus[].vram_total_mb) * 0.9; CPU-offload if ≤ ram_available_gb * 1024 * 0.9 — 0.9 is OS-reservation heuristic, TODO platform-specific calibration post-Plan-19; Disk if fits free disk; N/A otherwise); empty-state when no models; tok_s_estimated None when no GPU; DTO shape has tok_s_estimated: float | None.
- S4.3: Add tests/test_hardware_panel.py covering: SSE stream returns 200 with media_type text/event-stream and event: hardware framing; multi-GPU DTO; disk enumeration; generator terminates on disconnect; JSON serialization via model_dump(mode='json') handles Path/datetime/Enum.
