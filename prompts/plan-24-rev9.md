...
Open questions resolved: Q-24.1 (DIContainer factory semantics), Q-24.2 (GraphMemoryBackend wiring), Q-24.3 (build_context return type)
...
- DepartmentManager is stateless — all caches (SymbolMap, GraphMemoryBackend) are scoped per execute_task() call, not shared across calls.
...
- Abstract method: build_context(task) -> SymbolContext | None. Returns symbol context for the task, or None if unavailable. Catches SymbolMapUnavailableError: emit trace.department.context_build_failed event, return None. Do not fail the task — let the Worker proceed without symbol context.
- Document: Base DepartmentManager.execute_task() passes context dict to Worker. Subclasses MUST populate required keys. CodingManager adds 'graph_memory' and 'symbol_map' keys.
...
- build_context() override: retrieves SymbolMap via container.retrieve(SymbolMap), calls symbol_map.index(self._project_root), returns SymbolContext with ranked symbols. If SymbolMapUnavailableError, falls back to base class behavior (return None).
...
- Retrieves GraphMemoryBackend per task: graph_memory = container.retrieve(GraphMemoryBackend). Passes to Worker as memory parameter (NOT in context dict). ReActLoop.run(..., memory=graph_memory) receives it separately from prompt context.
...
- Test both paths: test_symbol_map_tree_sitter.py and test_symbol_map_degraded.py (verifies health_check returns DEGRADED when tree-sitter missing). Add test_symbol_map_latency_budget.py: measures SymbolMap(project_root=test_path) construction time ONLY. Constructor auto-indexes when project_root is provided. No separate index() call in the test. Budget is met if time ≤2000ms. If test is flaky on CI, mark as @pytest.mark.slow and run on dedicated runner.
...
- Constructor: __init__(self, db_path: Path | None = None). Default db_path is :memory: for per-task ephemeral storage. Future plans may pass a shared path for persistent memory.
...
- test_symbol_map_latency_budget.py: single-shot cold-start ≤2000ms.
- test_graph_memory.py: traversal + persistence, recursive CTE.
- test_department_full_cycle.py: wires mock DepartmentManager -> ReActLoop -> SkillRunner (file_search) -> EventBus -> Librarian. Verifies single task completes, emits structured events, Librarian stores at least one Observation. Use runtime guard inside test body (NOT @pytest.mark.skipif): def test_department_full_cycle(): if not all_deps_present(): pytest.skip('Requires Plans 21-24'). all_deps_present checks for sovereignai/agent/react.py, sovereignai/shared/event_bus.py (extended), and sovereignai/managers/coding.py. Use importlib.util.find_spec for module importability check, not os.path.exists. Remove guard after all plans land.
