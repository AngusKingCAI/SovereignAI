...
2. **Panel content loaders**:
   ...
   - Orchestrator panel: existing chat form from Plan 6, now POSTs to `/api/orchestrate` instead of `/api/tasks`.
...
4. **WebSocket trace handling** (updated):
...
async def get_workers():
    """Return all registered workers from the capability graph."""
    graph = container.retrieve(CapabilityGraph)
    workers = graph.find_providers(CapabilityCategory.WORKER, "*")
    return [{"id": w[0].id, "name": w[1].name, "status": "active"} for w in workers]

@app.get("/api/tasks")
async def get_tasks():
    """Return all tasks from the task state machine."""
    tsm = container.retrieve(TaskStateMachine)
    return tsm.list_tasks()  # Returns list of Task objects

@app.get("/api/hardware")
async def get_hardware():
    """Return current hardware probe results."""
    probe = container.retrieve(HardwareProbe)
    info = probe.probe()
    return {
...
Per OR48: These endpoints use only public API types (`CapabilityGraph`, `TaskStateMachine`, `HardwareProbe` are all registered in the container and retrieved by type — no direct imports of core internals).
...
- `tests/test_log_drawer.py`:
  - `test_trace_filter_by_level`: Mock WebSocket messages with different levels, apply filter, assert only matching levels visible.
...
Use `selenium` or `playwright` for JS-heavy tests? No — adds heavy dependency. Use FastAPI `TestClient` for HTML structure tests and unit-test the JS filter logic by extracting it to testable pure functions. For JS interaction, test via `pytest` with `js2py` or simply verify the DOM structure and event handlers are present.

**Decision**: Use `TestClient` for HTML structure. For JS logic, create `web/static/logic.js` with pure functions (filter, search) that can be tested via `js2py` or by porting to Python test fixtures. Keep it simple — the JS is vanilla and straightforward.
