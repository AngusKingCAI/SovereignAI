...
**Purpose**: Non-blocking validation scripts for Reviewer soft gates per AGENTS.md G6.
...
### Pattern Analysis Soft Gates
- `sg_pattern_count.py` - Warn if pattern analysis identified <3 high-frequency patterns
- `sg_cluster_quality.py` - Warn if ML clustering achieved silhouette score <0.5
- `sg_evidence_quality.py` - Warn if rule suggestions have weak evidence (<2 affected plans)

### Rule Integration Soft Gates
- Additional soft gates for rule integration workflow (future implementation)

## Usage Pattern

```bash
# Run soft gate (non-blocking, always returns 0)
python Agents/reviewer/scripts/soft_gates/sg_pattern_count.py
